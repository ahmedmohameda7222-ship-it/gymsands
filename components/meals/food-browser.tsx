...
function FoodBrowserInner({ initialLogs = emptyFoodLogs, onLogAdded, onPlanAdded, defaultMealType = "Breakfast", logDate }: FoodBrowserProps) {
  ...
  async function logQuickAdd() {
    ...
    try {
      const log = await quickAddManualFoodLog({ userId: user.id, date: logDate, mealType, calories: Number(quickAddDraft.calories), proteinG: Number(quickAddDraft.protein), notes: quickAddDraft.notes });
      pushLoggedFood(log);
