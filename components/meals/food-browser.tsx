...
import { AlertTriangle, BookOpen, CheckCircle2, Heart, Loader2, PlusCircle, RotateCcw, Search } from "lucide-react";
...
function fallbackKitchenData() {
  ...
  const subcategories = egyptianFoodSubcategories.map((name) => ({
  ...
  return { kitchens: [kitchen], subcategories };
}
        ...
        <Card className="border-warning/30 bg-warning/10">
          <CardContent className="space-y-3 pt-5 text-sm text-foreground">
            <div className="flex items-center gap-2 font-semibold">
              <AlertTriangle className="h-4 w-4" /> Food picker could not load
            </div>
            ...
            <Button variant="outline" className="min-h-12" onClick={() => this.setState({ message: null })}>
              Try again
            </Button>
          </CardContent>
        </Card>
...
function FoodBrowserInner({
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      .then(([kitchenResult, mealsResult, favoritesResult]) => {
        ...
        const kitchenData = kitchenResult.status === "fulfilled" ? kitchenResult.value : fallbackKitchenData();
        if (kitchenResult.status === "rejected") {
          messages.push(userSafeError(kitchenResult.reason, "Kitchen categories could not fully load. Default categories are shown."));
        }
        if (!user?.id) {
          messages.push("Signed-in custom foods, saved meals, and synced favorites are unavailable in this session.");
        }
        ...
        if (favoritesResult.status === "rejected") {
          messages.push(userSafeError(favoritesResult.reason, "Favorites could not sync. Favorite changes will retry when available."));
        }
  ...
  function setFoodAction(id: string, action: ActionKind, status: ActionStatus | null) {
    setFoodActions((current) => setActionState(current, id, action, status));
  }

  function setMealAction(id: string, action: ActionKind, status: ActionStatus | null) {
    setMealActions((current) => setActionState(current, id, action, status));
  }

  function foodAction(id: string, action: ActionKind) {
    return foodActions[actionKey(id, action)];
  }

  function mealAction(id: string, action: ActionKind) {
  ...
  async function logFoodNow(food: FoodItem) {
    ...
    if (validation) {
      setFoodAction(food.id, "log", { status: "error", label: "Check this food entry.", description: validation });
      return;
    }
    if (!user?.id) {
      setFoodAction(food.id, "log", { status: "error", label: "Login required.", description: "Please log in again before saving meals." });
      return;
    }
    ...
    try {
      ...
      setNotice({ type: "error", title: "Could not add food", description: userSafeError(error) });
    }
  }
  ...
  async function addToPlan(food: FoodItem) {
    ...
    const validation = validateFoodLogInput(food.food_name, quantity, macros);
    if (validation) {
      setFoodAction(food.id, "plan", { status: "error", label: "Check this food entry.", description: validation });
      return;
    }
    if (!user?.id) {
      setFoodAction(food.id, "plan", { status: "error", label: "Login required.", description: "Please log in again before saving your meal plan." });
      return;
    }
    ...
    try {
      ...
      onPlanAdded?.(item);
      const message = `${food.food_name} was added to ${displayMealType(mealType)}.`;
      setFoodAction(food.id, "plan", { status: "success", label: "Added to meal plan.", description: message });
      setNotice({ type: "success", title: "Added to My Meal Plan", description: message });
    } catch (error) {
      setFoodAction(food.id, "plan", { status: "error", label: "Could not add to plan.", description: userSafeError(error) });
      setNotice({ type: "error", title: "Could not add to plan", description: userSafeError(error) });
    }
  }

  async function logCustomMealNow(meal: CustomMeal) {
    if (isPending(mealAction(meal.id, "log"))) return;
    if (!user?.id) {
      setMealAction(meal.id, "log", { status: "error", label: "Login required.", description: "Please log in again before saving meals." });
      return;
    }
    setMealAction(meal.id, "log", { status: "pending", label: "Logging saved meal..." });
    try {
      const log = await addCustomMealToLog(user.id, meal, logDate, mealType);
      pushLoggedFood(log);
      const message = `${meal.meal_name} was added to ${displayMealType(mealType)}.`;
      setMealAction(meal.id, "log", { status: "success", label: "Saved meal logged.", description: message });
      setNotice({ type: "success", title: "Saved meal logged", description: message });
    } catch (error) {
      setMealAction(meal.id, "log", { status: "error", label: "Could not log saved meal.", description: userSafeError(error) });
      setNotice({ type: "error", title: "Could not log saved meal", description: userSafeError(error) });
    }
  }

  async function addCustomMealPlan(meal: CustomMeal) {
    if (isPending(mealAction(meal.id, "plan"))) return;
    if (!user?.id) {
      setMealAction(meal.id, "plan", { status: "error", label: "Login required.", description: "Please log in again before saving your meal plan." });
      return;
    }
    setMealAction(meal.id, "plan", { status: "pending", label: "Adding saved meal to plan..." });
    try {
      const item = await addCustomMealToMealPlan(user.id, meal, mealType);
      onPlanAdded?.(item);
      const message = `${meal.meal_name} was added to ${displayMealType(mealType)}.`;
      setMealAction(meal.id, "plan", { status: "success", label: "Added to meal plan.", description: message });
      setNotice({ type: "success", title: "Saved meal added to plan", description: message });
    } catch (error) {
      setMealAction(meal.id, "plan", { status: "error", label: "Could not add saved meal.", description: userSafeError(error) });
      setNotice({ type: "error", title: "Could not add saved meal", description: userSafeError(error) });
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass-card grid gap-3 p-4 sm:p-5 lg:grid-cols-[minmax(0,1fr)_190px_190px_auto_auto_auto_auto]">
        ...
        <select value={mealType} onChange={(event) => setMealType(event.target.value as MealType)} className={selectClassName} aria-label="Meal type">
          {mealOptions.map((type) => <option key={type} value={type}>{displayMealType(type)}</option>)}
        </select>
        <select value={selectedKitchen?.id ?? ""} onChange={(event) => selectKitchen(event.target.value)} className={selectClassName} aria-label="Kitchen">
          {kitchens.map((kitchen) => <option key={kitchen.id} value={kitchen.id}>{kitchen.name}</option>)}
        </select>
        <Button className="min-h-12" variant={favoritesOnly ? "default" : "outline"} onClick={() => setFavoritesOnly((current) => !current)}>
          <Heart className="h-4 w-4" /> Favorites
        </Button>
        <Button className="min-h-12" variant={libraryMode === "savedMeals" ? "default" : "outline"} onClick={() => setLibraryMode((current) => current === "foods" ? "savedMeals" : "foods")}>
          <BookOpen className="h-4 w-4" /> Saved Meals
        </Button>
        <Button asChild variant="outline" className="min-h-12">
          <Link href="/calories/food-hub?builder=1">Add Food/Meal</Link>
        </Button>
        <Button
          className="min-h-12"
          variant="outline"
          onClick={resetFilters}
          disabled={!query && !favoritesOnly && libraryMode === "foods" && selectedKitchen?.id === kitchens[0]?.id && selectedSubcategory?.id === visibleSubcategories[0]?.id}
        >
          <RotateCcw className="h-4 w-4" /> Reset
        </Button>
      </div>

      <SourceStatus isLoading={isLoadingKitchenData} messages={sourceStatusMessages} />

      {libraryMode === "foods" ? (
        <div className="glass-card p-3">
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
            <div>
              <p className="text-sm font-semibold text-foreground">{selectedKitchen?.name ?? "Kitchen"}</p>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 lg:flex-wrap lg:overflow-visible">
            ...
            {!visibleSubcategories.length ? (
              <p className="text-sm text-muted-foreground">No subcategories for this kitchen yet. Use search, recent foods, or create custom foods.</p>
            ) : null}
          </div>
        </div>
      ) : null}

      {libraryMode === "savedMeals" ? (
        isLoadingKitchenData ? (
          <CardGridSkeleton count={3} rows={3} />
        ) : (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {!customMeals.length ? (
              <div className="glass-card border-dashed p-4 text-sm text-muted-foreground">
                <p className="font-semibold text-foreground">No saved meals yet</p>
                <p className="mt-1">Create one in Food Builder, then log it here as a manual fallback or repeat meal.</p>
              </div>
            ) : null}
            {customMeals.map((meal) => {
              const planAction = mealAction(meal.id, "plan");
              const logAction = mealAction(meal.id, "log");
              return (
                <Card key={meal.id} variant="glass">
                  <CardContent className="pt-5">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <h3 className="font-semibold text-foreground">{meal.meal_name}</h3>
                        <p className="mt-1 text-sm text-muted-foreground">{meal.items.length} foods</p>
                      </div>
                    ...
                    <div className="mt-4 grid gap-2 sm:grid-cols-3">
                      ...
                      <Button className="min-h-12" onClick={() => logCustomMealNow(meal)} disabled={isPending(logAction)}>
                        {isPending(logAction) ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}
                        {isPending(logAction) ? "Logging..." : `Log to ${displayMealType(mealType)}`}
                      </Button>
                      <Button asChild className="min-h-12" variant="outline">
                        <Link href="/calories/food-hub?builder=1">Edit</Link>
                      </Button>
                    </div>
                    <ActionStatusList states={[planAction, logAction]} />
                  </CardContent>
                </Card>
              );
            })}
      ...
      {libraryMode === "foods" && isLoadingFoods ? <CardGridSkeleton count={3} rows={4} /> : null}

      {libraryMode === "foods" && !isLoadingFoods && !foods.length ? (
        <div className="solid-row border-dashed p-4 text-sm text-muted-foreground">
          <p className="font-semibold text-foreground">No foods found</p>
          <p className="mt-1">Try another kitchen, clear filters, use Add Food/Meal, or create a saved custom food/meal.</p>
        </div>
      ) : null}
                ...
                <CardContent className="pt-5">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <ConfidenceBadge source={confidenceForFood(food)} />
                      <FoodAccuracyBadges food={food} />
                    </div>
                  </div>
                  ...
                  <div className="mt-4 grid gap-2 sm:grid-cols-2">
                    ...
                    >
                      ...
                      {isPending(favoriteAction) ? "Saving..." : favorite ? "Favorited" : "Favorite food"}
                    </Button>
                  </div>
                  <ActionStatusList states={[planAction, logAction, favoriteAction]} />
                </CardContent>
              </Card>
            );
          })}
      ...
      {libraryMode === "foods" && foods.length > visibleCount && !favoritesOnly ? (
        <div className="flex justify-center">
          <Button className="min-h-12" type="button" variant="outline" onClick={() => setVisibleCount((current) => current + pageSize)}>
            Load more foods
          </Button>
        </div>
      ) : null}
  ...
  return (
    <div className={`rounded-md border p-4 text-sm ${styles}`}>
      <div className="flex items-start justify-between gap-3">
        ...
        <button type="button" onClick={onClose} className="min-h-12 min-w-12 rounded-md px-3 text-xs font-semibold underline">
          close
        </button>
      </div>
    </div>
...
function SourceStatus({ isLoading, messages }: { isLoading: boolean; messages: string[] }) {
  if (!isLoading && !messages.length) return null;
  return (
    <div className="rounded-md border border-border/70 bg-card/80 p-4 text-sm text-muted-foreground" aria-live="polite">
      {isLoading ? (
        <div className="flex items-center gap-2 text-foreground">
          <Loader2 className="h-4 w-4 animate-spin" />
          Loading kitchens, saved meals, and favorites...
        </div>
      ) : null}
      {messages.length ? (
...
function ActionStatusList({ states }: { states: Array<ActionStatus | undefined> }) {
  const visibleStates = states.filter(Boolean) as ActionStatus[];
  if (!visibleStates.length) return null;
  return (
    <div className="mt-3 space-y-2" aria-live="polite">
      {visibleStates.map((state) => (
        <div
          key={`${state.status}-${state.label}-${state.description ?? ""}`}
        ...
        >
          <p className="flex items-center gap-2 font-semibold">
            {state.status === "pending" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : state.status === "success" ? <CheckCircle2 className="h-3.5 w-3.5" /> : <AlertTriangle className="h-3.5 w-3.5" />}
            {state.label}
          </p>
          {state.description ? <p className="mt-1 text-muted-foreground">{state.description}</p> : null}
        </div>
      ))}
    </div>
  );
...
function Macro({ label, value }: { label: string | number; value: string | number }) {
  return (
...
function FoodAccuracyBadges({ food }: { food: FoodItem }) {
  ...
  return (
    <>
      {hasUnknownMacros(food) ? <Badge variant="outline">unknown macros</Badge> : null}
...
function customMealCategory(meal: CustomMeal) {
  const category = (meal as { meal_category?: unknown }).meal_category;
...
function confidenceForFood(food: FoodItem) {
  ...
  if (source.includes("user")) return "user-created";
...
function sourceLabelForFood(food: Pick<FoodItem, "source_type" | "is_global">) {
  ...
  if (source.toLowerCase().includes("verified")) return "verified source";
  if (source.toLowerCase().includes("import")) return "imported source";
...
function setActionState(current: ActionStateMap, id: string, action: ActionKind, status: ActionStatus | null) {
...
function isPending(status: ActionStatus | undefined) {
