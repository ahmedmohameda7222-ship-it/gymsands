...
function QuickAddPanel({ mealType, quickAdd, setQuickAdd, onQuickAdd }: { mealType: MealType; quickAdd: QuickAddState; setQuickAdd: Dispatch<SetStateAction<QuickAddState>>; onQuickAdd: () => void }) { return <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Zap className="h-5 w-5 text-primary" /> Quick/manual add</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-[1fr_1fr_1fr_1fr_1.4fr_auto]"><Input type="number" min="0" inputMode="decimal" enterKeyHint="done" value={quickAdd.calories} onChange={(e) => setQuickAdd((current) => ({ ...current, calories: e.target.value }))} placeholder="Calories" /><Input type="number" min="0" inputMode="decimal" enterKeyHint="done" value={quickAdd.protein} onChange={(e) => setQuickAdd((current) => ({ ...current, protein: e.target.value }))} placeholder="Protein g" /><Input type="number" min="0" inputMode="decimal" enterKeyHint="done" value={quickAdd.carbs} onChange={(e) => setQuickAdd((current) => ({ ...current, carbs: e.target.value }))} placeholder="Carbs g" /><Input type="number" min="0" inputMode="decimal" enterKeyHint="done" value={quickAdd.fat} onChange={(e) => setQuickAdd((current) => ({ ...current, fat: e.target.value }))} placeholder="Fat g" /><Input value={quickAdd.notes} onChange={(e) => setQuickAdd((current) => ({ ...current, notes: e.target.value }))} placeholder="Optional notes" /><Button onClick={onQuickAdd}><CheckCircle2 className="h-4 w-4" /> Add to {displayMealType(mealType)}</Button><p className="text-xs text-muted-foreground md:col-span-6">Quick add is an estimated manual entry, not verified nutrition data.</p></CardContent></Card>; }
