...
class FoodBrowserBoundary extends Component<{ children: ReactNode }, { message: string | null }> {
  ...
  static getDerivedStateFromError() { return { message: "Please refresh and try again." }; }
  render() {
...
function FoodBrowserInner({ initialLogs = emptyFoodLogs, onLogAdded, onPlanAdded, defaultMealType = "Breakfast", logDate }: FoodBrowserProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((error) => { if (active) setNotice({ type: "error", title: "Could not load kitchens", description: userSafeError(error, "Showing the default food categories.") }); })
  ...
  useEffect(() => {
    ...
    getFoodLibrary(user?.id ?? "", debouncedQuery, { kitchen: selectedKitchen?.name, kitchenId: selectedKitchen?.id, subcategoryId: selectedSubcategory?.id, category: selectedSubcategory?.name, limit: 90 })
      ...
      .catch((error) => {
        ...
        setNotice({ type: "error", title: "Could not load every food", description: userSafeError(error, "Showing available foods.") });
      })
  ...
  async function logFoodNow(food: FoodItem) {
    ...
    } catch (error) { setNotice({ type: "error", title: "Could not add food", description: userSafeError(error) }); }
  }
...
function ConfidenceBadge({ source }: { source: string }) { return <Badge variant={source === "verified" ? "success" : source === "imported" || source.includes("library") || source === "admin-reviewed" ? "default" : "outline"}>{source}</Badge>; }
