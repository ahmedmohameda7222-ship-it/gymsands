...
import { AlertTriangle, BookOpen, CheckCircle2, Clock, Heart, PlusCircle, RotateCcw, Save, Search, Trash2, Utensils, Zap } from "lucide-react";
...
const emptyQuickAdd = { calories: "", protein: "", carbs: "", fat: "", notes: "" };
...
type QuickAddState = typeof emptyQuickAdd;
...
class FoodBrowserBoundary extends Component<{ children: ReactNode }, { message: string | null }> {
  ...
  render() {
    if (this.state.message) {
      return <Card className="border-amber-200 bg-amber-50"><CardContent className="space-y-3 pt-5 text-sm text-amber-950"><div className="flex items-center gap-2 font-semibold"><AlertTriangle className="h-4 w-4" /> Food picker could not load</div><p>The food picker stayed open. Try again or choose another kitchen.</p><p className="break-words text-xs text-amber-800">{this.state.message}</p><Button variant="outline" size="sm" onClick={() => this.setState({ message: null })}>Try again</Button></CardContent></Card>;
    }
...
function FoodBrowserInner({ initialLogs = emptyFoodLogs, onLogAdded, onPlanAdded, defaultMealType = "Breakfast", logDate }: FoodBrowserProps) {
  ...
  const [quickAddDraft, setQuickAddDraft] = useState<QuickAddState>(emptyQuickAdd);
  ...
  async function logQuickAdd() {
    ...
    try {
      const log = await quickAddManualFoodLog({ userId: user.id, date: logDate, mealType, calories: Number(quickAddDraft.calories), proteinG: Number(quickAddDraft.protein), carbsG: Number(quickAddDraft.carbs), fatG: Number(quickAddDraft.fat), notes: quickAddDraft.notes });
      pushLoggedFood(log);
      setQuickAddDraft(emptyQuickAdd);
      setNotice({ type: "success", title: "Quick/manual entry logged", description: "This entry is labeled as estimated, not verified." });
    } catch (error) { setNotice({ type: "error", title: "Could not quick add", description: error instanceof Error ? error.message : "Please try again." }); }
  }
  ...
  function toggleFavoriteForKey(key: string, label: string) { const nextFavorite = !favoriteKeys.includes(key); setFavoriteKeys(setFavoriteFood(user?.id, key, nextFavorite)); setNotice({ type: "success", title: nextFavorite ? "Food favorited" : "Food unfavorited", description: label }); }
  ...
  function removeRecipe(recipe: SavedRecipe) { deleteRecipe(user?.id, recipe.id); setRecipes((current) => current.filter((item) => item.id !== recipe.id)); setNotice({ type: "success", title: "Recipe deleted", description: recipe.name }); }

  return <div className="space-y-4"><Card className="bg-blue-50"><CardContent className="pt-5"><p className="text-sm font-semibold text-foreground">{nutritionDisclaimer}</p><p className="mt-1 text-sm text-primary-foreground">Fast logging uses only saved foods, real food logs, and manual entries you create.</p></CardContent></Card>{notice ? <NoticeBox notice={notice} onClose={() => setNotice(null)} /> : null}<div className="grid gap-3 lg:grid-cols-[1fr_190px_190px_auto_auto]"><div className="relative"><Search className="absolute left-3 top-3 h-5 w-5 text-slate-400" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search food, e.g. chicken, rice, sauce" className="pl-10" /></div><select value={mealType} onChange={(event) => setMealType(event.target.value as MealType)} className="h-11 w-full rounded-md border bg-white px-3 text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-ring" aria-label="Meal type">{mealOptions.map((type) => <option key={type} value={type}>{displayMealType(type)}</option>)}</select><select value={selectedKitchen?.id ?? ""} onChange={(event) => selectKitchen(event.target.value)} className="h-11 w-full rounded-md border bg-white px-3 text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-ring" aria-label="Kitchen">{kitchens.map((kitchen) => <option key={kitchen.id} value={kitchen.id}>{kitchen.name}</option>)}</select><Button variant={favoritesOnly ? "default" : "outline"} onClick={() => setFavoritesOnly((current) => !current)}><Heart className="h-4 w-4" /> Favorites</Button><Button variant="outline" onClick={resetFilters} disabled={!query && !favoritesOnly && selectedKitchen?.id === kitchens[0]?.id && selectedSubcategory?.id === visibleSubcategories[0]?.id}><RotateCcw className="h-4 w-4" /> Reset</Button></div><div className="rounded-md border bg-white p-3"><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-semibold text-slate-950">{selectedKitchen?.name ?? "Kitchen"}</p><p className="text-xs text-muted-foreground">{isLoadingKitchenData ? "Loading kitchen data..." : `${visibleSubcategories.length} subcategories available`}</p></div>{selectedSubcategory ? <span className="text-xs font-semibold text-primary">{selectedSubcategory.name}</span> : null}</div><div className="flex gap-2 overflow-x-auto pb-1 lg:flex-wrap lg:overflow-visible">{visibleSubcategories.map((subcategory) => <Button key={subcategory.id} type="button" variant={selectedSubcategory?.id === subcategory.id ? "default" : "outline"} size="sm" onClick={() => setSelectedSubcategoryId((current) => (current === subcategory.id ? "" : subcategory.id))} className="shrink-0">{subcategory.name}</Button>)}{!visibleSubcategories.length ? <p className="text-sm text-muted-foreground">No subcategories for this kitchen yet.</p> : null}</div></div><QuickAddPanel mealType={mealType} quickAdd={quickAddDraft} setQuickAdd={setQuickAddDraft} onQuickAdd={logQuickAdd} /><SpeedPanels recentFoods={recentFoods} frequentFoods={frequentFoods} favoriteLogs={favoriteLogs} favoriteKeys={favoriteKeys} onLog={logPrevious} onFavorite={toggleFavoriteForKey} onRefresh={() => loadSpeedData().catch(() => undefined)} /><SavedMealsPanel customMeals={customMeals} mealType={mealType} onLog={logCustomMealNow} onPlan={addCustomMealPlan} /><RecipePanel recipeDraft={recipeDraft} setRecipeDraft={setRecipeDraft} ingredientDraft={ingredientDraft} setIngredientDraft={setIngredientDraft} ingredients={recipeIngredients} setIngredients={setRecipeIngredients} recipes={recipes} onAddIngredient={addIngredient} onSaveRecipe={saveCurrentRecipe} onLogRecipe={logRecipe} onDeleteRecipe={removeRecipe} />{isLoadingFoods ? <p className="text-sm text-muted-foreground">Loading foods...</p> : null}{!isLoadingFoods && !foods.length ? <p className="text-sm text-muted-foreground">No foods found. Try another kitchen, subcategory, or search word.</p> : null}<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{visibleFoods.map((food) => { const quantity = quantities[food.id] ?? 1; const selectedUnit = units[food.id] ?? "serving"; const macros = scaleFoodMacros(food, quantity); const favoriteKey = favoriteKeyForFood(food); const favorite = favoriteKeys.includes(favoriteKey); return <Card key={food.id}><CardContent className="pt-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="font-semibold text-slate-950">{food.food_name}</h3><p className="mt-1 text-sm text-muted-foreground">{food.serving_size}</p></div><div className="flex flex-col items-end gap-2"><Badge>{food.category || "Food"}</Badge><ConfidenceBadge source={confidenceForFood(food)} /></div></div><p className="mt-2 text-xs text-muted-foreground">{food.cuisine || selectedKitchen?.name || egyptianFoodKitchenName}</p><div className="mt-4 grid grid-cols-4 gap-2 text-center"><Macro label="kcal" value={macros.calories} /><Macro label="protein" value={`${macros.protein_g}g`} /><Macro label="carbs" value={`${macros.carbs_g}g`} /><Macro label="fat" value={`${macros.fat_g}g`} /></div><div className="mt-4 grid gap-2 sm:grid-cols-[1fr_140px]"><div><label htmlFor={`quantity-${food.id}`} className="mb-2 block text-sm font-medium text-slate-800">Quantity</label><Input id={`quantity-${food.id}`} type="number" min="0.1" step="0.1" value={quantity} onChange={(event) => setQuantities((current) => ({ ...current, [food.id]: Math.max(0.1, Number(event.target.value) || 1) }))} placeholder="1" /></div><div><label htmlFor={`unit-${food.id}`} className="mb-2 block text-sm font-medium text-slate-800">Unit</label><select id={`unit-${food.id}`} value={selectedUnit} onChange={(event) => setUnits((current) => ({ ...current, [food.id]: event.target.value as ServingUnit }))} className="h-10 w-full rounded-md border bg-white px-3 text-sm">{servingUnits.map((unit) => <option key={unit} value={unit}>{unit}</option>)}</select></div></div><div className="mt-4 grid gap-2 sm:grid-cols-2"><Button type="button" variant="outline" onClick={() => addToPlan(food)}><PlusCircle className="h-4 w-4" /> Add to plan</Button><Button type="button" onClick={() => logFoodNow(food)}><CheckCircle2 className="h-4 w-4" /> Done now</Button><Button className="sm:col-span-2" type="button" variant={favorite ? "default" : "outline"} onClick={() => toggleFavoriteForKey(favoriteKey, food.food_name)}><Heart className="h-4 w-4" /> {favorite ? "Favorited" : "Favorite food"}</Button></div></CardContent></Card>; })}</div>{foods.length > visibleCount && !favoritesOnly ? <div className="flex justify-center"><Button type="button" variant="outline" onClick={() => setVisibleCount((current) => current + pageSize)}>Load more foods</Button></div> : null}</div>;
}

function QuickAddPanel({ mealType, quickAdd, setQuickAdd, onQuickAdd }: { mealType: MealType; quickAdd: QuickAddState; setQuickAdd: Dispatch<SetStateAction<QuickAddState>>; onQuickAdd: () => void }) { return <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Zap className="h-5 w-5 text-primary" /> Quick/manual add</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-[1fr_1fr_1fr_1fr_1.4fr_auto]"><Input type="number" min="0" value={quickAdd.calories} onChange={(e) => setQuickAdd((current) => ({ ...current, calories: e.target.value }))} placeholder="Calories" /><Input type="number" min="0" value={quickAdd.protein} onChange={(e) => setQuickAdd((current) => ({ ...current, protein: e.target.value }))} placeholder="Protein g" /><Input type="number" min="0" value={quickAdd.carbs} onChange={(e) => setQuickAdd((current) => ({ ...current, carbs: e.target.value }))} placeholder="Carbs g" /><Input type="number" min="0" value={quickAdd.fat} onChange={(e) => setQuickAdd((current) => ({ ...current, fat: e.target.value }))} placeholder="Fat g" /><Input value={quickAdd.notes} onChange={(e) => setQuickAdd((current) => ({ ...current, notes: e.target.value }))} placeholder="Optional notes" /><Button onClick={onQuickAdd}><CheckCircle2 className="h-4 w-4" /> Add to {displayMealType(mealType)}</Button><p className="text-xs text-muted-foreground md:col-span-6">Quick add is an estimated manual entry, not verified nutrition data.</p></CardContent></Card>; }
