...
import { AlertTriangle, BookOpen, CheckCircle2, Heart, PlusCircle, RotateCcw, Save, Search, Trash2 } from "lucide-react";
...
function FoodBrowserInner({ initialLogs = emptyFoodLogs, onLogAdded, onPlanAdded, defaultMealType = "Breakfast", logDate }: FoodBrowserProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([kitchenData, meals, favorites, recipes]) => {
        if (!active) return;
