...
function metricRemaining(metric: NutritionMetric, energyUnit: UserAppSettings["energyUnit"], remainingLabel: string, aboveLabel: string) {
  ...
  const amount = metric.key === "calories" ? formatEnergy(Math.abs(metric.remaining), energyUnit) : `${Math.round(Math.abs(metric.remaining) * 10) / 10} g`;
...
export function EatNutritionProgress({ metrics, activeTarget, selectedDate, energyUnit, onRetryTargets }: {
  ...
  return <Card className="overflow-hidden border-primary/20"><CardContent className="space-y-4 p-4 sm:p-5">
    <div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-semibold text-foreground">{targetLabel}{target?.values.daily_calories ? ` · ${formatEnergy(target.values.daily_calories, energyUnit)}` : ""}</p>{activeTarget.status === "failed" ? <p className="mt-1 text-xs text-destructive">{et("targetsFailed")}</p> : null}</div>{activeTarget.status === "failed" ? <Button type="button" variant="outline" size="sm" onClick={onRetryTargets}><RefreshCcw className="h-4 w-4" />{et("retry")}</Button> : <Button asChild type="button" variant="outline" size="sm"><Link href={manageHref}>{target?.hasTarget ? et("manage") : et("setTarget")}</Link></Button>}</div>
    <div className="space-y-2"><div className="flex items-end justify-between gap-3"><div><p className="text-3xl font-extrabold tracking-tight">{metricValue(calories, energyUnit)}{calories.target !== null ? <span className="text-base font-medium text-muted-foreground"> / {metricTarget(calories, energyUnit)}</span> : null}</p><p className="mt-1 text-sm text-muted-foreground">{metricRemaining(calories, energyUnit, et("remaining"), et("aboveTarget"))}</p></div><Badge variant={calories.state === "materially-over" ? "destructive" : calories.state === "over" ? "warning" : calories.state === "near" ? "success" : "outline"}>{calories.state === "unavailable" ? et("unavailable") : calories.state === "near" ? et("targetHit") : `${Math.round(calories.percent ?? 0)}%`}</Badge></div><Progress value={Math.min(100, calories.percent ?? 0)} className={progressClass(calories.state)} aria-label={`${et("calories")} ${calories.percent ?? 0}%`} /></div>
    <div className="grid gap-2 sm:grid-cols-3">{metrics.filter((metric) => metric.key !== "calories").map((metric) => <div key={metric.key} className="rounded-[14px] border border-border/70 bg-muted/25 p-3"><div className="flex items-center justify-between gap-2"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{metricName(metric.key, et)}</p><span className="text-xs text-muted-foreground">{metric.percent === null ? "—" : `${Math.round(metric.percent)}%`}</span></div><p className="mt-1 text-sm font-semibold">{metricValue(metric, energyUnit)}{metric.target !== null ? ` / ${metricTarget(metric, energyUnit)}` : ""}</p><Progress value={Math.min(100, metric.percent ?? 0)} className={`mt-2 ${progressClass(metric.state)}`} aria-label={`${metricName(metric.key, et)} ${metric.percent ?? 0}%`} /></div>)}</div>
  </CardContent></Card>;
}

export function PlannedNextMeal({ item, pending, onMarkEaten, onAdjust, onReplace }: {
...
export function CompactHydration({ water, waterTargetMl, liquidUnit, pending, feedback, onAdd, onRetry }: {
  ...
  return <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="flex items-center gap-2 text-base"><Droplets className="h-4 w-4 text-primary" />{et("water")}</CardTitle>{water.status === "loaded" ? <Button asChild variant="ghost" size="sm"><Link href="/hydration">{et("more")}</Link></Button> : null}</CardHeader><CardContent className="space-y-3">{water.status === "failed" ? <div className="flex items-center justify-between gap-2"><p className="text-sm text-destructive">{et("hydrationUnavailable")}</p><Button variant="outline" size="sm" onClick={onRetry}><RefreshCcw className="h-4 w-4" />{et("retry")}</Button></div> : water.status === "loading" ? <p className="text-sm text-muted-foreground">{et("loadingLogs")}</p> : <><p className="text-2xl font-bold">{formatLiquid(total ?? 0, liquidUnit)}{waterTargetMl ? <span className="text-sm font-medium text-muted-foreground"> / {formatLiquid(waterTargetMl, liquidUnit)}</span> : null}</p><div className="grid grid-cols-2 gap-2"><Button variant="outline" className="min-h-12" onClick={() => onAdd(250)} disabled={pending}>+{formatLiquid(250, liquidUnit)}</Button><Button variant="outline" className="min-h-12" onClick={() => onAdd(500)} disabled={pending}>+{formatLiquid(500, liquidUnit)}</Button></div><InlineFeedback message={feedback} /></>}</CardContent></Card>;
}
