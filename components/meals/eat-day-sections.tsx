...
function MetricProgress({ metric, label }: { metric: NutritionMetric; label: string }) {
  if (metric.percent === null) return <div className="h-2 w-full rounded-full bg-muted" aria-label={`${label} unavailable`} />;
  return <Progress value={Math.min(100, metric.percent)} className={progressClass(metric.state)} aria-label={`${label} ${metric.percent}%`} />;
}
...
export function EatNutritionProgress({ metrics, activeTarget, selectedDate, energyUnit, onRetryTargets }: {
  ...
  return <Card className="overflow-hidden border-primary/20"><CardContent className="space-y-4 p-4 sm:p-5">
    ...
    <div className="space-y-2"><div className="flex items-end justify-between gap-3"><div><p className="text-3xl font-extrabold tracking-tight">{metricValue(calories, energyUnit, locale)}{calories.target !== null ? <span className="text-base font-medium text-muted-foreground"> / {metricTarget(calories, energyUnit, locale)}</span> : null}</p><p className="mt-1 text-sm text-muted-foreground">{metricRemaining(calories, energyUnit, locale, et("remaining"), et("aboveTarget"))}</p></div><Badge variant={calories.state === "materially-over" ? "destructive" : calories.state === "over" ? "warning" : calories.state === "near" ? "success" : "outline"}>{calories.state === "unavailable" ? et("unavailable") : calories.state === "near" ? et("targetHit") : calories.percent === null ? "—" : `${Math.round(calories.percent)}%`}</Badge></div><MetricProgress metric={calories} label={et("calories")} /></div>
    <div className="grid gap-2 sm:grid-cols-3">{metrics.filter((metric) => metric.key !== "calories").map((metric) => <div key={metric.key} className="rounded-[14px] border border-border/70 bg-muted/25 p-3"><div className="flex items-center justify-between gap-2"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{metricName(metric.key, et)}</p><span className="text-xs text-muted-foreground">{metric.percent === null ? "—" : `${Math.round(metric.percent)}%`}</span></div><p className="mt-1 text-sm font-semibold">{metricValue(metric, energyUnit, locale)}{metric.target !== null ? ` / ${metricTarget(metric, energyUnit, locale)}` : ""}</p><div className="mt-2"><MetricProgress metric={metric} label={metricName(metric.key, et)} /></div></div>)}</div>
  </CardContent></Card>;
}
...
export function PlannedNextMeal({ item, pending, chatGptPending, energyUnit, onMarkEaten, onAdjust, onReplace }: {
  ...
  if (!item) return null;
  return <Card><CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-base"><Utensils className="h-4 w-4 text-primary" />{et("plannedNextMeal")}</CardTitle></CardHeader><CardContent className="space-y-3"><div><p className="font-semibold text-foreground">{item.food_name}</p><p className="mt-1 text-sm text-muted-foreground">{mealLabel(item.meal_type)} · {nullableEnergy(item.calories, energyUnit, locale)} · {item.quantity} × {item.serving_size}</p></div><div className="grid gap-2 sm:grid-cols-3"><Button className="min-h-12" onClick={() => onMarkEaten(item)} disabled={pending || chatGptPending}><CheckCircle2 className="h-4 w-4" />{pending ? et("logging") : et("markEaten")}</Button><Button className="min-h-12" variant="outline" onClick={() => onAdjust(item)} disabled={pending || chatGptPending}>{et("adjustFirst")}</Button><Button className="min-h-12" variant="outline" onClick={() => onReplace(item)} disabled={pending || chatGptPending}>{chatGptPending ? <RotateCcw className="h-4 w-4 animate-spin" /> : <OpenAiBlossom className="h-4 w-4" />}{ert("adjustWithChatGpt")}</Button></div></CardContent></Card>;
}
