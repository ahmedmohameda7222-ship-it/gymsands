...
type AdjustForm = Omit<EatFoodLogPatch, "calories"> & { energy: number };

function draft(item: MealPlanItem, energyUnit: UserAppSettings["energyUnit"]): AdjustForm {
  return { foodName: item.food_name, quantity: Number(item.quantity) || 1, servingSize: item.serving_size, mealType: item.meal_type, energy: eatEnergyDisplayValue(Number(item.calories) || 0, energyUnit), proteinG: Number(item.protein_g) || 0, carbsG: Number(item.carbs_g) || 0, fatG: Number(item.fat_g) || 0, notes: item.notes };
}
...
export function EatPlannedMealAdjust({
  ...
  function confirm() {
    if (!form) return;
    onConfirm({ ...form, calories: eatEnergyInputToKcal(form.energy, energyUnit) }, updatePlan);
  }
