...
export function NutritionTargetSettings({ selectedDate, returnHref }: { selectedDate: string; returnHref: string }) {
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [et, profile?.age, profile?.gender, profile?.height_cm, profile?.weight_kg, selectedDate, settings.weightUnit, user?.id]);
  ...
  function useEstimate(targetType: NutritionTargetProfileType | "fallback") {
