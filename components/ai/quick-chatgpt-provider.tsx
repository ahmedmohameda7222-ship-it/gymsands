...
import { buildRuntimePrompt, filterRuntimeLibrary, getRuntimeContextChips, getRuntimeHomeSections, localizePrompt, RUNTIME_QUICK_PROMPTS, rankRuntimePrompts } from "@/lib/ai/prompt-runtime";
import { type PromptCapability, type PromptCategory, type PromptLanguage, type QuickPromptContext, type QuickPromptDefinition } from "@/lib/ai/quick-prompts";
...
type QuickChatGptContextValue = { openPrompts: (promptId?: string) => void; openCustomPrompt: (prompt: CustomQuickPrompt) => void; setDashboardContext: (context: QuickPromptContext) => void; rankedPrompts: QuickPromptDefinition[]; dashboardContext: QuickPromptContext; isOpen: boolean };
const QuickChatGptContext = createContext<QuickChatGptContextValue | null>(null);
const emptyContext: QuickPromptContext = { nutrition: { hasTargets: false, targetsState: "loading", foodLogsState: "loading", foodLogCount: null, mealPlanCount: null }, grocery: { state: "loading", itemCount: null }, hydration: { state: "loading", logCount: null }, recovery: { state: "loading", hasData: false }, wellness: { state: "loading", habitCount: null, supplementCount: null }, progress: { state: "loading", entryCount: null }, profile: { state: "loading" } };
...
export function QuickChatGptProvider({ children }: { children: ReactNode }) {
  ...
  const openPrompts = useCallback((promptId?: string) => {
    ...
    setView(promptId && RUNTIME_QUICK_PROMPTS.some((prompt) => prompt.id === promptId) ? { name: "detail", promptId, backTo: "home" } : { name: "home" });
    setOpen(true);
  }, []);
  ...
  function closeSurface(next: boolean) { setOpen(next); if (!next) { setView({ name: "home" }); setCustomPrompt(null); setEditedPrompt(""); setCopyError(false); setLibrarySearch(""); setLibraryCategory("all"); } }
