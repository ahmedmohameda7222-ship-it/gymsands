...
export function WorkoutPlanBuilder({ onSaved }: { loadActivePlan?: boolean; onSaved?: () => void | Promise<void> }) {
  ...
  useEffect(() => {
    if (hydrated) storeJson(draftKey, draft);
  ...
  const scheduleValid = Boolean(
    draft.days.length &&
    draft.days.every((day) => day.dayName.trim() && day.weekday) &&
    new Set(draft.days.map((day) => day.weekday)).size === draft.days.length
  );
  ...
  function patchDay(index: number, patch: Partial<BuilderDay>) {
    setDraft((current) => ({
      ...current,
      days: current.days.map((day, itemIndex) => itemIndex === index ? { ...day, ...patch } : day)
    }));
  }

  function patchExercise(dayIndex: number, exerciseIndex: number, patch: Partial<Workout>) {
    setDraft((current) => ({
      ...current,
  ...
  function addExercises(workouts: Workout[]) {
    ...
    patchDay(activeDayIndex, {
      exercises: [
        ...activeDay.exercises,
        ...workouts.filter((workout) => !existing.has(exerciseKey(workout))).map(withDefaults)
      ]
    });
  }
  ...
  return (
    <div className="space-y-6" dir={dir} data-train-builder>
          ...
          return (
            <li key={label} aria-current={active ? "step" : undefined} data-step-state={complete ? "complete" : active ? "current" : "upcoming"} className={`flex items-center border-e border-border/80 px-2.5 py-2 last:border-e-0 sm:px-3 ${active ? "bg-primary/10" : complete ? "bg-success/10" : "bg-card"}`}>
              <span className="flex items-center gap-2 text-xs font-semibold sm:text-sm">
                {complete ? <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-success text-success-foreground"><Check className="h-3.5 w-3.5" /></span> : <span className={`grid h-6 w-6 shrink-0 place-items-center rounded-full border text-xs ${active ? "border-primary text-primary" : "border-border text-muted-foreground"}`}>{number}</span>}
        ...
        <div className="space-y-6" data-builder-step="days">
          ...
          <div className="grid grid-flow-col auto-cols-[240px] gap-2 overflow-x-auto pb-2" aria-label={tr("trainingDaysHeading")} data-builder-day-tabs>
            {draft.days.map((day, index) => {
              ...
              return (
                <button key={`${day.weekday}-${index}`} type="button" aria-pressed={selected} onClick={() => setActiveDayIndex(index)} className={`min-h-[72px] rounded-2xl border px-4 py-2 text-start focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${selected ? "border-primary bg-primary/10 ring-1 ring-primary/20" : validationAttempted && incomplete ? "border-warning/50 bg-warning/5" : "border-border/70 bg-card"}`}>
                  <span className="block truncate font-semibold">{day.dayName}</span>
                  <span className="mt-1 inline-flex items-center gap-1.5 text-xs font-medium text-muted-foreground">{incomplete ? tr("incomplete") : <><Check className="h-3.5 w-3.5 text-success" />{tr("complete")}</>}</span>
                  <span className="ms-1 text-xs text-muted-foreground">· {tr("exercises", { count: day.exercises.length })}</span>
                </button>
              );
            })}
          ...
          <Card data-day-configuration>
            <CardContent className="grid gap-4 p-4 sm:p-5 lg:grid-cols-[minmax(0,1fr)_220px_auto] lg:items-end">
              ...
              <div className="flex items-center justify-end gap-1">
                <span className="grid h-11 w-11 place-items-center text-muted-foreground" aria-hidden="true"><GripVertical className="h-5 w-5" /></span>
                <ActionMenu label={`${tr("moreActions")}: ${activeDay.dayName}`} visibleLabel={tr("moreActions")} icon={<MoreHorizontal className="h-4 w-4" />}>
                  <ActionMenuItem onSelect={() => moveDay(activeDayIndex, -1)} disabled={activeDayIndex === 0}><ArrowUp className="me-2 inline h-4 w-4" />{tr("moveUp", { name: activeDay.dayName })}</ActionMenuItem>
                  <ActionMenuItem onSelect={() => moveDay(activeDayIndex, 1)} disabled={activeDayIndex === draft.days.length - 1}><ArrowDown className="me-2 inline h-4 w-4" />{tr("moveDown", { name: activeDay.dayName })}</ActionMenuItem>
                  <ActionMenuItem destructive onSelect={() => removeDay(activeDayIndex)} disabled={draft.days.length <= 1}><Trash2 className="me-2 inline h-4 w-4" />{tr("removeItem", { name: activeDay.dayName })}</ActionMenuItem>
                </ActionMenu>
              </div>
          ...
          <section className="space-y-3" aria-labelledby="selected-exercises-heading">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div><h3 id="selected-exercises-heading" className="text-lg font-semibold">{tr("selectedExercises")}</h3><p className="text-sm text-muted-foreground">{tr("prescriptionHelp")}</p></div>
              <Button type="button" variant="outline" className="min-h-11" onClick={() => setPickerOpen(true)}><Plus className="h-4 w-4" /> {tr("addExercises")}</Button>
            </div>
              ...
              {activeDay.exercises.map((exercise, index) => (
                <article key={`${identity(exercise)}-${index}`} className="rounded-2xl border border-border/70 bg-card p-4" data-exercise-prescription>
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-3 xl:grid-cols-[88px_112px_112px_minmax(180px,1fr)_132px] xl:items-end">
                    ...
                    <div className="col-span-3 flex gap-1 xl:col-span-1 xl:justify-end">
                      <Button variant="ghost" size="icon" className="min-h-11 min-w-11" aria-label={tr("moveUp", { name: exercise.name })} onClick={() => moveExercise(index, -1)} disabled={index === 0}><ArrowUp className="h-4 w-4" /></Button>
                      <Button variant="ghost" size="icon" className="min-h-11 min-w-11" aria-label={tr("moveDown", { name: exercise.name })} onClick={() => moveExercise(index, 1)} disabled={index === activeDay.exercises.length - 1}><ArrowDown className="h-4 w-4" /></Button>
                      <Button variant="ghost" size="icon" className="min-h-11 min-w-11 text-destructive" aria-label={tr("removeItem", { name: exercise.name })} onClick={() => removeExercise(index)}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </div>
                </article>
              ))}
          ...
          <PlanMuscleLoadPanel days={draft.days} activeDayIndex={activeDayIndex} defaultScope="current_day" variant="builder" language={language} />
      ...
      {step === 3 ? (
        <div className="space-y-4" data-builder-step="review">
          <Card className="border-primary/30 bg-primary/5">
            <CardContent className="p-5">
              <p className="text-sm text-muted-foreground">{tr("reviewStep")}</p>
              <h2 className="mt-1 text-2xl font-semibold">{draft.planName}</h2>
              {draft.description ? <p className="mt-2 text-sm text-muted-foreground">{draft.description}</p> : null}
              <div className="mt-3 flex flex-wrap gap-2">
                <Badge>{tr("trainingDays", { count: draft.days.length })}</Badge>
                <Badge variant="outline">{tr("exercises", { count: totalExercises })}</Badge>
                {draft.durationWeeks ? <Badge variant="outline">{tr("programWeeks", { count: draft.durationWeeks })}</Badge> : null}
                {draft.sessionMinutes ? <Badge variant="outline">{tr("aboutMinutes", { count: draft.sessionMinutes })}</Badge> : null}
                {draft.goal ? <Badge variant="outline">{draft.goal}</Badge> : null}
              </div>
            </CardContent>
          </Card>
          <div className="grid gap-3 md:grid-cols-2">
            {draft.days.map((day) => (
              <Card key={`${day.weekday}-${day.dayName}`}>
                <CardContent className="p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div><p className="text-sm text-muted-foreground">{day.weekday ? new Intl.DateTimeFormat(locale, { weekday: "long" }).format(new Date(2024, 0, 7 + weekDays.indexOf(day.weekday))) : tr("unscheduled")}</p><h3 className="font-semibold">{day.dayName}</h3>{day.notes ? <p className="mt-1 text-sm text-muted-foreground">{day.notes}</p> : null}</div>
                    <Badge variant="outline">{tr("exercises", { count: day.exercises.length })}</Badge>
                  </div>
                  <ol className="mt-3 space-y-2">
                    {day.exercises.map((exercise, index) => (
                      <li key={`${identity(exercise)}-${index}`} className="flex justify-between gap-3 text-sm">
                        <span>{index + 1}. {exercise.name}{userFacingExerciseNote(exercise.notes) ? <span className="block text-xs text-muted-foreground">{userFacingExerciseNote(exercise.notes)}</span> : null}</span>
                        <span className="text-end text-muted-foreground">{exercise.sets ?? 3} × {exercise.reps ?? "8–12"}<span className="block text-xs">{tr("secondsRest", { count: exercise.rest_seconds ?? 75 })}</span></span>
                      </li>
                    ))}
                  </ol>
                </CardContent>
          ...
          <PlanMuscleLoadPanel days={draft.days} activeDayIndex={activeDayIndex} defaultScope="entire_plan" variant="review" language={language} />
