...
import { buildTodayActions, enabledQuickLogs, hasCurrentDayActivity, quickLogRoutes, resolveTodayWorkoutState, selectRelevantMeal, type TodayAction, type TodayWorkoutState } from "@/lib/dashboard/today-model";
...
import { getGroceryItems, getNutritionTargetProfiles, upsertGroceryItem } from "@/services/database/execution-layer";
...
import type { FitnessHabit, FoodLog, MealPlanItem, OnboardingAnswers, ProgressEntry, SleepRecoveryLog, SupplementLog, UserGroceryItem, UserWorkoutPlan, UserWorkoutPlanDay, WaterLog, WorkoutSession } from "@/types";
...
type NutritionData = { logs: FoodLog[]; targets: SavedTargets | null; activeTarget: ActiveNutritionTarget | null };
type WellnessData = { habits: FitnessHabit[]; supplements: SupplementLog[]; sleepLogs: SleepRecoveryLog[]; partialErrors: string[] };
...
export function TodayDashboard() {
  ...
  const [nutritionData, setNutritionData] = useState<NutritionData>({ logs: [], targets: null, activeTarget: null });
  const [waterLogs, setWaterLogs] = useState<WaterLog[]>([]);
  const [groceryItems, setGroceryItems] = useState<UserGroceryItem[]>([]);
  const [wellnessData, setWellnessData] = useState<WellnessData>({ habits: [], supplements: [], sleepLogs: [], partialErrors: [] });
  ...
  const loadDashboard = useCallback(async () => {
    ...
    const loadNutrition = async (): Promise<NutritionData> => {
      const [logsResult, targetResult, profilesResult, planResult] = await Promise.allSettled([getTodayFoodLogs(user.id), getCalorieTargets(user.id), user.id === "mock-user" ? Promise.resolve([]) : getNutritionTargetProfiles(user.id), getDefaultUserWorkoutPlan(user.id)]);
      if (logsResult.status === "rejected" && targetResult.status === "rejected") throw logsResult.reason;
      ...
      const resolved = resolveActiveNutritionTarget({ profiles, baseTarget: base, requestedType: override === "auto" ? automaticType : override });
      return { logs, targets: resolved.hasTarget ? resolved.values : null, activeTarget: resolved };
    };
    const loadWellness = async (): Promise<WellnessData> => {
      const results = await Promise.allSettled([getFitnessHabits(user.id, today), getSupplementLogs(user.id), getSleepRecoveryLogs(user.id, 7)]);
      ...
      return { habits: results[0].status === "fulfilled" ? results[0].value : [], supplements: results[1].status === "fulfilled" ? results[1].value : [], sleepLogs: results[2].status === "fulfilled" ? results[2].value : [], partialErrors };
    };
    ...
    results.forEach((result, index) => {
      ...
      if (source === "nutrition") setNutritionData(result.value as NutritionData);
  ...
  const totals = useMemo(() => sumFoodLogs(nutritionData.logs), [nutritionData.logs]);
  const targets = nutritionData.targets;
  const remaining = targets ? remainingMacros({ calories: targets.daily_calories, protein_g: targets.protein_g, carbs_g: targets.carbs_g, fat_g: targets.fat_g, water_ml: targets.water_ml }, totals) : null;
  const waterTotal = useMemo(() => waterLogs.reduce((sum, item) => sum + Number(item.amount_ml), 0), [waterLogs]);
  const workoutState = resolveTodayWorkoutState({ today, planDayId: workoutData.day?.id ?? null, openSessionId: workoutData.openSessionId, sessions: workoutData.sessions });
  ...
  const actions = buildTodayActions({ workoutState, workoutTitle: workoutData.day?.day_name, workoutHref: workoutData.day ? `/workouts/session/day/${workoutData.day.id}` : null, workoutExerciseCount: workoutData.day?.exercises.length, workoutDurationMinutes: workoutData.plan?.session_duration_minutes, relevantMeal, foodLogCount: nutritionData.logs.length, remainingProtein: remaining?.protein_g ?? null, waterRemainingMl: targets?.water_ml ? Math.max(0, targets.water_ml - waterTotal) : null, checkinAvailable: true });
  ...
  const currentActivity = hasCurrentDayActivity({ foodLogCount: nutritionData.logs.length, waterLogCount: waterLogs.length, mealItems, workoutState, habitsCount: wellnessData.habits.length, supplementsCount: wellnessData.supplements.length, sleepLoggedToday: Boolean(todaySleep) });
  const setupChecklist = [
    ...
    { label: tt("addMealPlan"), done: mealItems.length > 0 || nutritionData.logs.length > 0, href: "/my-meal-plan" },
    { label: tt("addProgress"), done: setupData.progress.length > 0, href: "/progress" },
    { label: tt("startFirstWorkout"), done: workoutData.sessions.length > 0, href: workoutData.day ? `/workouts/session/day/${workoutData.day.id}` : "/my-workout/plans" }
  ];
...
const foodLogCount = nutritionData.logs.length;
...
const promptContext: QuickPromptContext = {
  ...
  nutrition: { hasTargets, remainingCalories, remainingProtein, foodLogCount, mealPlanCount },
...
}, [endOfWeek, foodLogCount, groceryItemCount, hasTargets, mealPlanCount, poorRecovery, remainingCalories, remainingProtein, setDashboardContext, sleepHours, today, workoutDurationMinutes, workoutExerciseCount, workoutState, workoutTitle]);
const dashboardPrompts = rankQuickPrompts(promptContext).slice(0, 4);
  ...
  async function markMealDone(item: MealPlanItem) {
    if (pendingMealIds.has(item.id)) return;
    setPendingMealIds((current) => new Set(current).add(item.id));
    try { const result = await markDirectMealPlanItemDone(item); setMealItems((current) => current.map((value) => value.id === item.id ? result.item : value)); if (result.log) setNutritionData((current) => ({ ...current, logs: [result.log!, ...current.logs] })); setFeedback(tt("mealSaved")); }
    catch (error) { toast({ title: tt("mealDoneFailed"), description: userSafeError(error), variant: "error" }); }
    finally { setPendingMealIds((current) => { const next = new Set(current); next.delete(item.id); return next; }); }
  }
  ...
  return <div dir={dir}>
    ...
    {isInitialLoading ? <CardGridSkeleton count={4} rows={3} className="xl:grid-cols-4" /> : <div className="space-y-4">
      ...
      <section aria-labelledby="today-progress"><div className="mb-2 flex items-center justify-between gap-2"><h2 id="today-progress" className="text-base font-semibold">{tt("todayProgress")}</h2>{nutritionData.activeTarget?.hasTarget ? <span className="text-xs text-muted-foreground">{tt("targetLabel", { label: nutritionData.activeTarget.label })}</span> : null}</div><div className="grid grid-cols-2 gap-3 xl:grid-cols-4"><ProgressMetric icon={<Flame className="h-5 w-5" />} label={tt("calories")} value={formatEnergy(totals.calories, settings.energyUnit)} detail={targets ? `${formatEnergy(targets.daily_calories, settings.energyUnit)} ${tt("targetLabel", { label: "" }).trim()}` : tt("noTarget")} progress={targets?.daily_calories ? percent(totals.calories, targets.daily_calories) : undefined} /><ProgressMetric icon={<Soup className="h-5 w-5" />} label={tt("protein")} value={`${Math.round(totals.protein_g)} g`} detail={targets?.protein_g ? `${Math.round(targets.protein_g)} g` : tt("noTarget")} progress={targets?.protein_g ? percent(totals.protein_g, targets.protein_g) : undefined} /><ProgressMetric icon={<Droplets className="h-5 w-5" />} label={tt("water")} value={formatLiquid(waterTotal, settings.liquidUnit)} detail={targets?.water_ml ? formatLiquid(targets.water_ml, settings.liquidUnit) : tt("noTarget")} progress={targets?.water_ml ? percent(waterTotal, targets.water_ml) : undefined} /><ProgressMetric icon={<Dumbbell className="h-5 w-5" />} label={tt("workout")} value={workoutState === "active" ? tt("active") : workoutState === "completed" ? tt("completed") : workoutState === "scheduled" ? tt("notStarted") : tt("noWorkout")} detail={workoutData.day?.day_name ?? tt("restDay")} /></div></section>

      <section aria-labelledby="today-plan"><h2 id="today-plan" className="mb-2 text-base font-semibold">{tt("todayPlan")}</h2><div className="grid gap-4 lg:grid-cols-2"><Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Dumbbell className="h-5 w-5" />{tt("todaysWorkout")}</CardTitle></CardHeader><CardContent className="space-y-3">{workoutData.day ? <><div><p className="font-semibold">{workoutData.day.day_name}</p><p className="text-sm text-muted-foreground">{tt("exercisesCount", { count: workoutData.day.exercises.length })}{workoutData.plan?.session_duration_minutes ? ` · ${tt("durationMinutes", { minutes: workoutData.plan.session_duration_minutes })}` : ""}</p></div><div className="space-y-1">{workoutData.day.exercises.slice(0, 3).map((exercise) => <p key={exercise.id} className="text-sm text-muted-foreground">{exercise.sets ?? 1} × {exercise.reps ?? "?"} {exercise.exercise_name}</p>)}</div><Button asChild variant="outline" className="min-h-12"><Link href={`/workouts/session/day/${workoutData.day.id}`}>{workoutState === "active" ? tt("resumeWorkout") : workoutState === "completed" ? tt("viewWorkout") : tt("startWorkout")}</Link></Button></> : <p className="text-sm text-muted-foreground">{tt("noWorkoutScheduled")}</p>}</CardContent></Card><Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Utensils className="h-5 w-5" />{tt("todaysMeals")}</CardTitle></CardHeader><CardContent className="space-y-3">{relevantMeal ? <><div><p className="font-semibold">{mealTypeLabel(relevantMeal.meal_type, tt)}: {relevantMeal.food_name}</p><p className="text-sm text-muted-foreground">{Math.round(relevantMeal.calories)} kcal · {Math.round(relevantMeal.protein_g)} g {tt("protein")}</p></div><div className="flex flex-wrap gap-2"><Button type="button" onClick={() => void markMealDone(relevantMeal)} disabled={pendingMealIds.has(relevantMeal.id)} className="min-h-12"><Check className="h-4 w-4" />{pendingMealIds.has(relevantMeal.id) ? tt("saving") : tt("markDone")}</Button><Button type="button" variant="outline" onClick={() => void skipMealItems([relevantMeal])} disabled={pendingMealIds.has(relevantMeal.id)} className="min-h-12">{tt("skip")}</Button><Button type="button" variant="ghost" onClick={() => void skipMealItems(mealItems.filter((item) => item.status === "planned"))} disabled={!mealItems.some((item) => item.status === "planned")} className="min-h-12">{tt("skipAll")}</Button></div></> : mealItems.some((item) => item.status === "skipped") ? <p className="text-sm text-muted-foreground">{tt("skipped")}</p> : <p className="text-sm text-muted-foreground">{tt("noMealsPlanned")}</p>}<Button asChild variant="ghost" className="min-h-11"><Link href={`/my-meal-plan?tab=day&date=${today}`}>{tt("openMealPlan")}</Link></Button></CardContent></Card></div></section>
      ...
      {!currentActivity && !isInitialLoading ? <EmptyState title={tt("noActivityToday")} description={tt("noActivityDetail")} actionLabel={tt("quickLog")} actionHref="/calories" /> : null}
    </div>}
  </div>;
}
