...
import { Activity, MapPin, RefreshCcw, Smartphone, Watch } from "lucide-react";
...
export function ConnectedApps() {
  ...
  async function calculateRoute() {
    ...
    const response = await fetch("/api/maps/routes", {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ origin: { address: origin } })
    });
    const data = await response.json().catch(() => ({}));
    setIsBusy(null);
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="rounded-md border p-3">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Connect and import completed cardio activities.</p>
            <div className="mt-3 flex flex-wrap gap-2">
              <Button onClick={connectStrava} disabled={isBusy === "strava"}><Activity className="h-4 w-4" /> Connect</Button>
              <Button variant="outline" onClick={importStrava} disabled={isBusy === "strava-import"}><RefreshCcw className="h-4 w-4" /> Import</Button>
            </div>
          </div>
          <div className="rounded-md border p-3">
            <p className="font-semibold">Google Health</p>
            <p className="mt-1 text-sm text-muted-foreground">OAuth-ready. Import is feature-gated until API scopes are approved.</p>
            <Button className="mt-3" variant="outline" onClick={connectGoogleHealth} disabled={isBusy === "google-health"}>Connect placeholder</Button>
          </div>
          <div className="rounded-md border p-3">
            <p className="font-semibold">Health Connect</p>
            <p className="mt-1 text-sm text-muted-foreground">Health Connect is available only in the future Android app.</p>
          </div>
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><MapPin className="h-5 w-5 text-primary" /> Gym Distance</CardTitle>
          <CardDescription>Google Maps Routes estimates travel distance and time when configured.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <Input value={origin} onChange={(event) => setOrigin(event.target.value)} placeholder="Enter your starting address" />
          <Button onClick={calculateRoute} disabled={!origin.trim() || isBusy === "maps"}><MapPin className="h-4 w-4" /> Calculate route</Button>
          {routeText ? <p className="rounded-md border bg-card p-3 text-sm font-semibold">{routeText}</p> : null}
          <div className="flex items-start gap-2 rounded-md border p-3 text-sm text-muted-foreground">
            ...
            Browser location can be added later by passing latitude and longitude to the same server route.
          </div>
        </CardContent>
      </Card>
    </div>
