...
export function MobileFloatingNav({ pathname }: { pathname: string }) {
  ...
  const visibleQuickLogs = settings.quickLogSections;
  return (
      ...
      <nav className="relative grid h-[72px] grid-cols-5 items-stretch rounded-[24px] border border-border/80 bg-card/92 px-1 shadow-[var(--shadow-floating)] backdrop-blur-md" aria-label={tt("primaryMobileNavigation")}>
        ...
        <Dialog>
          <DialogTrigger asChild>
            <button type="button" aria-label={tt("quickLog")} className="relative flex min-h-12 flex-col items-center justify-end gap-1 pb-2 text-[10px] font-semibold text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
              <span className="absolute -top-4 flex h-14 w-14 items-center justify-center rounded-full border-4 border-[var(--app-bg)] bg-primary text-primary-foreground shadow-[var(--shadow-floating)]"><Plus className="h-6 w-6" /></span>
              <span>{tt("quickLog")}</span>
            </button>
          </DialogTrigger>
          <DialogContent variant="glass" className="inset-x-0 bottom-0 left-0 top-auto max-h-[85dvh] w-full max-w-full translate-x-0 translate-y-0 rounded-b-none rounded-t-[24px] p-0">
            ...
            <div className="grid gap-2 p-4 pb-[calc(env(safe-area-inset-bottom)+1.25rem)]">
              {visibleQuickLogs.map((section) => <DialogClose key={section} asChild><Link href={quickLogRoutes[section]} className="flex min-h-12 items-center rounded-[14px] border border-border/70 bg-card px-3 text-sm font-semibold">{tt(quickLogLabels[section])}</Link></DialogClose>)}
        ...
        <button type="button" aria-pressed={isOpen} onClick={() => openPrompts()} className={cn("flex min-h-12 flex-col items-center justify-center gap-1 rounded-[18px] px-1 text-[10px] font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", isOpen ? "text-primary" : "text-muted-foreground")}>
          <OpenAiBlossom className="h-5 w-5" /><span className="max-w-full truncate">{tt("chatGpt")}</span>
        </button>
      </nav>
    </div>
  );
...
function RouteItem({ href, label, icon, active }: { href: string; label: string; icon: React.ReactNode; active: boolean }) {
  return <Link href={href} aria-current={active ? "page" : undefined} className={cn("flex min-h-12 flex-col items-center justify-center gap-1 rounded-[18px] px-1 text-[10px] font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", active ? "text-primary" : "text-muted-foreground hover:text-primary")}>{icon}<span className="max-w-full truncate">{label}</span></Link>;
}
