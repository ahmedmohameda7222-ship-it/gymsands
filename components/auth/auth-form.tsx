...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  async function handleGoogleSignIn() {
    ...
    if (mode === "register") {
      ...
      if (explicitNext) {
        window.localStorage.setItem("plaivra.oauth.next", safeInternalRedirectPath(explicitNext));
      }
      window.localStorage.setItem("plaivra.oauth.mode", "login");
    }
          ...
          <div className="space-y-3 mb-4">
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-card px-2 text-muted-foreground">{copy.or}</span>
              </div>
            </div>
          </div>
        ) : null}
