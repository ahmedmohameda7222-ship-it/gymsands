...
import { useNutritionV1Translation, type NutritionV1Key } from "@/lib/i18n/nutrition-v1";
import {
...
const weekdayOptions = [
  ...
  { value: 4, key: "thursday" }, { value: 5, key: "friday" }, { value: 6, key: "saturday" }, { value: 7, key: "sunday" },
] as const satisfies ReadonlyArray<{ value: number; key: NutritionV1Key }>;
...
function label(date: string, options: Intl.DateTimeFormatOptions, locale?: string) { return new Intl.DateTimeFormat(locale, options).format(new Date(`${date}T12:00:00`)); }
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 py-5 sm:px-6" dir={dir} lang={language}>
      <header className="border-b border-border pb-5">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-semibold tracking-tight">{nt("mealPlan")}</h1><p className="mt-1 text-sm text-muted-foreground">{nt("mealPlanDescription")}</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => void planWithChatGPT()} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-border px-3 text-sm font-medium">{nt("planWithChatGpt")} <ExternalLink className="h-4 w-4" /></button><Link href={`/my-meal-plan/shopping?week=${encodeURIComponent(weekStart)}&date=${encodeURIComponent(selectedDate)}`} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-foreground px-3 text-sm font-semibold text-background"><ShoppingCart className="h-4 w-4" />{nt("shopping")}</Link></div></div>
        <div className="mt-5 flex items-center justify-between gap-3"><button type="button" onClick={() => shiftWeek(-7)} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-border" aria-label={language === "ar" ? "الأسبوع السابق" : language === "de" ? "Vorherige Woche" : "Previous week"}><ChevronLeft className="h-4 w-4" /></button><div className="text-center"><p className="text-sm font-semibold">{label(weekStart, { month: "short", day: "numeric" }, locale)} – {label(shift(weekStart, 6), { month: "short", day: "numeric", year: "numeric" }, locale)}</p><p className="text-xs text-muted-foreground">{nt("weekRevision", { revision: data?.week?.revision ?? 0 })}</p></div><button type="button" onClick={() => shiftWeek(7)} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-border" aria-label={language === "ar" ? "الأسبوع التالي" : language === "de" ? "Nächste Woche" : "Next week"}><ChevronRight className="h-4 w-4" /></button></div>
      ...
      {data ? <>
        <div className="py-5"><div className="flex flex-wrap items-baseline justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{language === "ar" ? "اليوم المحدد" : language === "de" ? "Ausgewählter Tag" : "Selected day"}</p><h2 className="mt-1 text-xl font-semibold">{label(selectedDate, { weekday: "long", month: "long", day: "numeric" }, locale)}</h2></div><div className="flex flex-wrap gap-2">{selectedDate !== today ? <button type="button" onClick={() => selectDate(today)} className="min-h-11 rounded-xl px-3 text-sm font-medium hover:bg-muted">{nt("today")}</button> : null}<button type="button" onClick={() => setBulkCopyDraft({ scope: "day", targetDate: shift(selectedDate, 1) })} className="min-h-11 rounded-xl px-3 text-sm font-medium hover:bg-muted">{language === "ar" ? "نسخ اليوم" : language === "de" ? "Tag kopieren" : "Copy day"}</button><button type="button" onClick={() => setAddingCustomSlot(true)} className="inline-flex min-h-11 items-center gap-1 rounded-xl px-3 text-sm font-medium hover:bg-muted"><Plus className="h-4 w-4" />{language === "ar" ? "إضافة خانة وجبة" : language === "de" ? "Mahlzeiten-Slot hinzufügen" : "Add meal slot"}</button></div></div></div>
        ...
        <div aria-label={language === "ar" ? "وجبات اليوم المحدد" : language === "de" ? "Mahlzeiten des ausgewählten Tages" : "Selected day meals"}>{slots.map((slot) => <MealSlotSection key={slot} label={slot} items={selectedOccurrences.filter((item) => item.meal_slot_key === slot)} allowExecution={allowExecution} markEatenLabel={markEatenLabel} logWithChangesLabel={logWithChangesLabel} onAdd={() => setAddSlot(slot)} onMarkEaten={(item) => void markEaten(item)} onLogWithChanges={logWithChanges} onSkip={(item) => void skip(item)} onEdit={beginEdit} onMove={beginMove} onCopy={beginCopy} />)}</div>
        {!selectedOccurrences.length ? <div className="py-6 text-center"><p className="font-medium">{language === "ar" ? "لا توجد وجبات مخططة لهذا اليوم." : language === "de" ? "Für diesen Tag ist nichts geplant." : "Nothing planned for this day."}</p><p className="mt-1 text-sm text-muted-foreground">{language === "ar" ? "أضف يدويًا أو استخدم ChatGPT كمساعد تخطيط خارجي." : language === "de" ? "Füge manuell hinzu oder nutze ChatGPT als externen Planungsbeschleuniger." : "Add manually or use ChatGPT as an external planning accelerator."}</p><button type="button" onClick={() => setAddSlot("Breakfast")} className="mt-3 min-h-11 rounded-xl bg-foreground px-4 text-sm font-semibold text-background">{language === "ar" ? "إضافة إلى الخطة" : language === "de" ? "Zum Plan hinzufügen" : "Add to plan"}</button></div> : null}
      </> : null}
