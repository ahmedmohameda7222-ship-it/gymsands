...
function newRecipeOperationStorageKey(ownerId: string, source: RecipeFoodSource) {
  return ["plaivra", "nutrition", "handoff", ownerId, "recipe", "new", source.source, source.id, String(source.quantity), source.serving, source.displayName ?? "", source.languageTag ?? ""]
...
function commandSourceIdentity(source: AddToHandoffSource, resolvedRecipeQuantity: number | null) {
  if (source.type === "food") {
    return ["food", source.source, source.id, String(source.quantity), source.serving, source.displayName ?? "", source.languageTag ?? ""];
  }
