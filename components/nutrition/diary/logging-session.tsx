...
const loggerCopy = {
  en: {
    ...
    searchUnavailable: "Food search is temporarily unavailable.", servingUnavailable: "This Food has no authoritative serving yet. Choose a Food with an explicit serving before logging.", recipesUnavailable: "Recipes are temporarily unavailable.", addedToPlate: "{name} added to Plate.", enterValidBarcode: "Enter a valid barcode.",
  ...
  de: {
    ...
    searchUnavailable: "Die Lebensmittelsuche ist vorübergehend nicht verfügbar.", servingUnavailable: "Für dieses Lebensmittel gibt es noch keine verbindliche Portion. Wähle vor dem Protokollieren ein Lebensmittel mit expliziter Portion.", recipesUnavailable: "Rezepte sind vorübergehend nicht verfügbar.", addedToPlate: "{name} zum Teller hinzugefügt.", enterValidBarcode: "Gib einen gültigen Barcode ein.",
  ...
  ar: {
    ...
    searchUnavailable: "بحث الأطعمة غير متاح مؤقتًا.", servingUnavailable: "لا توجد حصة معتمدة لهذا الطعام حتى الآن. اختر طعامًا له حصة صريحة قبل التسجيل.", recipesUnavailable: "الوصفات غير متاحة مؤقتًا.", addedToPlate: "تمت إضافة {name} إلى الطبق.", enterValidBarcode: "أدخل باركود صالحًا.",
...
export function LoggingSession({ date, meal, savedMeals, plannedOccurrence = null, onClose, onConfirmed }: { date: string; meal: string; savedMeals: DiarySavedMealChoice[]; plannedOccurrence?: DiaryPlannedOccurrence | null; onClose: () => void; onConfirmed: () => void; }) {
  ...
  async function addBarcodeFood() {
    ...
    if (barcodeFood.source === "catalog" && barcodeFood.foodId && barcodeFood.servingSize) {
      try {
        const params = new URLSearchParams({
          source: "catalog",
          quantity: "1",
          serving: barcodeFood.servingSize,
    ...
    const nutrition = {
      caloriesKcal: known(barcodeFood.calories),
      proteinG: known(barcodeFood.protein),
      carbsG: known(barcodeFood.carbs),
      fatG: known(barcodeFood.fat),
    };
    pushItem({
      foodName: barcodeFood.name,
      servingLabel: barcodeFood.servingSize?.trim() || text.serving,
      quantity: 1,
      nutrition,
      source: {
        type: "food",
        ...
        frozenSnapshot: {
          ...
          servingLabel: barcodeFood.servingSize ?? null,
          nutrition,
        },
      },
    });
