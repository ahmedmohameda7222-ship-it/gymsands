...
const loggerCopy = {
  en: {
    ...
    searchUnavailable: "Food search is temporarily unavailable.", servingUnavailable: "This Food has no authoritative serving yet. Choose a Food with an explicit serving before logging.", chooseServing: "Choose an authoritative serving before adding.", providerSuggestion: "Provider barcode results are suggestion-only. Search for an existing Food or create My Food before logging.", recipesUnavailable: "Recipes are temporarily unavailable.", addedToPlate: "{name} added to Plate.", enterValidBarcode: "Enter a valid barcode.",
  ...
  de: {
    ...
    searchUnavailable: "Die Lebensmittelsuche ist vorübergehend nicht verfügbar.", servingUnavailable: "Für dieses Lebensmittel gibt es noch keine verbindliche Portion. Wähle vor dem Protokollieren ein Lebensmittel mit expliziter Portion.", chooseServing: "Wähle vor dem Hinzufügen eine verbindliche Portion.", providerSuggestion: "Barcode-Ergebnisse des Anbieters sind nur Vorschläge. Suche ein vorhandenes Lebensmittel oder erstelle Mein Lebensmittel vor dem Protokollieren.", recipesUnavailable: "Rezepte sind vorübergehend nicht verfügbar.", addedToPlate: "{name} zum Teller hinzugefügt.", enterValidBarcode: "Gib einen gültigen Barcode ein.",
  ...
  ar: {
    ...
    searchUnavailable: "بحث الأطعمة غير متاح مؤقتًا.", servingUnavailable: "لا توجد حصة معتمدة لهذا الطعام حتى الآن. اختر طعامًا له حصة صريحة قبل التسجيل.", chooseServing: "اختر حصة معتمدة قبل الإضافة.", providerSuggestion: "نتائج الباركود من المزود هي اقتراحات فقط. ابحث عن طعام موجود أو أنشئ طعامي قبل التسجيل.", recipesUnavailable: "الوصفات غير متاحة مؤقتًا.", addedToPlate: "تمت إضافة {name} إلى الطبق.", enterValidBarcode: "أدخل باركود صالحًا.",
...
export function LoggingSession({ date, meal, savedMeals, plannedOccurrence = null, onClose, onConfirmed }: { date: string; meal: string; savedMeals: DiarySavedMealChoice[]; plannedOccurrence?: DiaryPlannedOccurrence | null; onClose: () => void; onConfirmed: () => void; }) {
  ...
  const [barcodeServingKey, setBarcodeServingKey] = useState("");
  ...
  async function loadCatalogServingState(food: FoodLibraryCandidate) {
    const key = catalogServingStateKey(food);
    const cached = catalogServingStates[key];
    ...
    const data = await response.json().catch(() => ({})) as { servingChoices?: CatalogServingChoice[]; error?: string };
    if (!response.ok || !Array.isArray(data.servingChoices)) {
      throw new Error(data.error || text.servingUnavailable);
    }
    const state: CatalogServingState = {
      choices: data.servingChoices,
      selectedKey: data.servingChoices.length === 1 ? servingChoiceKey(data.servingChoices[0]!) : "",
    };
    setCatalogServingStates((current) => ({ ...current, [key]: state }));
    return state;
  }

  async function addCatalogFood(food: FoodLibraryCandidate, choice: CatalogServingChoice) {
    const params = new URLSearchParams({
      source: "catalog",
      quantity: "1",
      serving: choice.label,
      displayName: food.name,
      languageTag: food.locale,
    });
    if (choice.servingOptionId) params.set("servingOptionId", choice.servingOptionId);
    const response = await fetch(
      `/api/nutrition/v1/foods/${encodeURIComponent(food.id)}/handoff?${params.toString()}`,
      { headers: authHeaders(token) },
    );
    const handoff = await response.json().catch(() => ({})) as BarcodeCatalogHandoff & { error?: string };
    if (!response.ok || !handoff.foodId) throw new Error(handoff.error || text.servingUnavailable);
    pushItem({
      foodName: handoff.diaryItem.foodName,
      servingLabel: handoff.diaryItem.servingLabel,
      quantity: handoff.diaryItem.quantity,
      nutrition: handoff.diaryItem.nutrition,
      foodItemId: handoff.diaryItem.foodItemId,
      userFoodItemId: handoff.diaryItem.userFoodItemId,
      source: { type: "food", id: handoff.foodId, frozenSnapshot: handoff.frozenSourceSnapshot },
    });
  }

  async function addFood(food: FoodLibraryCandidate) {
    if (food.source === "my_food") {
      if (!food.servingLabel) { setFeedback(text.servingUnavailable); return; }
      ...
      pushItem({ foodName: food.name, servingLabel: food.servingLabel, quantity: 1, nutrition, foodItemId: null, userFoodItemId: food.id, source: { type: "food", id: food.id, frozenSnapshot } });
      return;
    }

    try {
      const state = await loadCatalogServingState(food);
      if (state.choices.length === 0) { setFeedback(text.servingUnavailable); return; }
      const selected = state.choices.find((choice) => servingChoiceKey(choice) === state.selectedKey)
        ?? (state.choices.length === 1 ? state.choices[0]! : null);
      if (!selected) { setFeedback(text.chooseServing); return; }
      await addCatalogFood(food, selected);
    } catch (cause) {
      setFeedback(cause instanceof Error ? cause.message : text.servingUnavailable);
    }
  }
  ...
  async function lookupBarcode() {
    ...
    try {
      const response = await fetch(
        `/api/food/open-food-facts?barcode=${encodeURIComponent(clean)}&locale=${encodeURIComponent(language)}`,
        { headers: authHeaders(token) },
      );
      ...
      const choices = data.food.source === "catalog" && Array.isArray(data.food.servingChoices)
        ? data.food.servingChoices
        : [];
      setBarcodeServingKey(choices.length === 1 ? servingChoiceKey(choices[0]!) : "");
      setFeedback(data.food.source === "provider_suggestion" ? text.providerSuggestion : text.productFound);
    } catch {
      setBarcodeFood(null);
      setFeedback(text.barcodeFallback);
    }
  }
  async function addBarcodeFood() {
    if (!barcodeFood) return;
    if (barcodeFood.source === "provider_suggestion") {
      setFeedback(text.providerSuggestion);
      return;
    }
    if (barcodeFood.source !== "catalog" || !barcodeFood.foodId) {
      setFeedback(text.barcodeFallback);
      return;
    }

    const choices = Array.isArray(barcodeFood.servingChoices) ? barcodeFood.servingChoices : [];
    if (choices.length === 0) {
      setFeedback(text.servingUnavailable);
      return;
    }
    const selected = choices.find((choice) => servingChoiceKey(choice) === barcodeServingKey)
      ?? (choices.length === 1 ? choices[0]! : null);
    if (!selected) {
      setFeedback(text.chooseServing);
      return;
    }
    ...
    try {
      const params = new URLSearchParams({
        ...
        displayName: barcodeFood.name,
        languageTag: barcodeFood.locale || language,
      });
      if (selected.servingOptionId) params.set("servingOptionId", selected.servingOptionId);
      const response = await fetch(
        `/api/nutrition/v1/foods/${encodeURIComponent(barcodeFood.foodId)}/handoff?${params.toString()}`,
        { headers: authHeaders(token) },
      );
      const handoff = await response.json().catch(() => ({})) as BarcodeCatalogHandoff;
      if (!response.ok || !handoff.foodId) throw new Error();
      pushItem({
        foodName: handoff.diaryItem.foodName,
        servingLabel: handoff.diaryItem.servingLabel,
        quantity: handoff.diaryItem.quantity,
        ...
        source: {
          type: "food",
          id: handoff.foodId,
          frozenSnapshot: handoff.frozenSourceSnapshot,
        },
      });
    } catch {
      setFeedback(text.barcodeFallback);
  ...
  const barcodeChoices = barcodeFood?.source === "catalog" && Array.isArray(barcodeFood.servingChoices)
    ? barcodeFood.servingChoices
    : [];
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          {mode === "search" ? <section className="space-y-3"><label className="flex min-h-12 items-center gap-2 rounded-xl border border-border px-3"><Search className="h-4 w-4 text-muted-foreground" /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder={et("searchFoods")} className="w-full bg-transparent text-sm outline-none" /></label>{searching && !foods.length ? <p className="text-sm text-muted-foreground">{et("loading")}</p> : <div className="divide-y divide-border">{foods.map((food) => {
              ...
              return <div key={`${food.source}:${food.id}`} className="flex min-h-[72px] items-center gap-3 py-2">
                <div className="min-w-0 flex-1">
                  ...
                  >
                    <option value="">{text.chooseServing}</option>
                    {servingState.choices.map((choice) => <option key={servingChoiceKey(choice)} value={servingChoiceKey(choice)}>{choice.label}</option>)}
                  </select> : null}
                  {food.source === "catalog" && catalogUnavailable ? <p className="mt-2 text-xs text-destructive">{text.servingUnavailable}</p> : null}
                  {food.source === "catalog" && catalogAmbiguous ? <p className="mt-2 text-xs text-muted-foreground">{text.chooseServing}</p> : null}
                </div>
                <button
                  type="button"
                  disabled={food.source === "my_food" ? !food.servingLabel : Boolean(catalogUnavailable || catalogAmbiguous)}
                  onClick={() => void addFood(food)}
                  className="min-h-11 rounded-xl border border-border px-3 text-sm font-medium hover:bg-muted disabled:cursor-not-allowed disabled:opacity-40"
                >{et("add")}</button>
              </div>;
            })}</div>}</section> : null}
          {mode === "barcode" ? <section className="space-y-3"><h3 className="font-semibold">{et("barcode")}</h3><p className="text-sm text-muted-foreground">{text.scanOrEnter}</p><div className="flex gap-2"><input inputMode="numeric" value={barcode} onChange={(event) => { setBarcode(event.target.value.replace(/\D/g, "")); setBarcodeFood(null); setBarcodeServingKey(""); }} placeholder={et("barcodePlaceholder")} className="min-h-12 flex-1 rounded-xl border border-border bg-background px-3 text-sm" /><button type="button" onClick={() => void lookupBarcode()} className="min-h-12 rounded-xl border border-border px-4 text-sm font-medium">{et("lookup")}</button></div>{barcodeFood ? <div className="space-y-3 border-y border-border py-3">
              ...
              {barcodeFood.source === "catalog" && barcodeChoices.length > 1 ? <select
                aria-label="Barcode authoritative serving"
                value={barcodeServingKey}
                onChange={(event) => setBarcodeServingKey(event.target.value)}
                className="min-h-11 w-full rounded-xl border border-border bg-background px-3 text-sm"
              >
                <option value="">{text.chooseServing}</option>
                {barcodeChoices.map((choice) => <option key={servingChoiceKey(choice)} value={servingChoiceKey(choice)}>{choice.label}</option>)}
              </select> : null}
              {barcodeFood.source === "catalog" && barcodeChoices.length === 0 ? <p className="text-sm text-destructive">{text.servingUnavailable}</p> : null}
              {barcodeFood.source === "provider_suggestion" ? <p className="text-sm text-muted-foreground">{text.providerSuggestion}</p> : null}
