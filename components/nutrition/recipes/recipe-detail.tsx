...
const ownerCacheKey = (ownerId: string, recipeId: string) => `plaivra:nutrition:recipe:${ownerId}:${recipeId}:published`;
const legacyCacheKey = (recipeId: string) => `plaivra:nutrition:recipe:${recipeId}:published`;
...
export function RecipeDetail({ recipeId }: { recipeId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    window.localStorage.removeItem(legacyCacheKey(recipeId));
    try {
      ...
      if (ownerId && snapshot) window.localStorage.setItem(ownerCacheKey(ownerId, recipeId), serializePublishedRecipeCache(snapshot));
    } catch (cause) {
      const snapshot = ownerId ? parsePublishedRecipeCache(window.localStorage.getItem(ownerCacheKey(ownerId, recipeId))) : null;
      const trustedSnapshot = snapshot && snapshot.recipeId === recipeId && typeof snapshot.recipeVersionId === "string" && snapshot.recipeVersionId.length > 0 ? snapshot : null;
      if (trustedSnapshot) {
        setDetail(cachedDetail(trustedSnapshot));
        setDetailOwnerId(ownerId);
        setCached(true);
      } else {
        setError(cause instanceof Error ? cause.message : nt("recipeLoadFailed"));
      }
    } finally {
      setLoading(false);
    }
  ...
  const visibleDetail = detailOwnerId === ownerId ? detail : null;
  const snapshot = useMemo(() => visibleDetail ? toSnapshot(visibleDetail) : null, [visibleDetail]);
  const isPublished = Boolean(visibleDetail?.latestVersion);
  ...
  if (authLoading || loading) return <div className="mx-auto max-w-[720px] space-y-3 px-4 py-6"><div className="h-10 animate-pulse rounded-xl bg-muted" /><div className="h-44 animate-pulse rounded-2xl bg-muted" /></div>;
    ...
    <article dir={dir} className="mx-auto w-full max-w-[720px] space-y-6 px-4 py-5 sm:px-6">
      <header className="space-y-4 border-b border-border/70 pb-5">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="flex items-center gap-2"><h1 className="text-2xl font-semibold tracking-tight"><bdi dir="auto">{version.name}</bdi></h1>{visibleDetail.root.is_favorite ? <Star className="h-4 w-4 fill-current" aria-label={nt("favorite")} /> : null}</div>
            <p className="mt-1 text-sm text-muted-foreground">{nt("published")} · {nt("servingsCount", { count: version.servings })}{version.total_time_minutes !== null ? ` · ${nt("minutesShort", { count: version.total_time_minutes })}` : ""}{visibleDetail.cuisine ? <> · <bdi dir="auto">{visibleDetail.cuisine}</bdi></> : null}</p>
            {cached ? <p className="mt-1 text-xs font-medium text-amber-700 dark:text-amber-300">{nt("offlineCachedRecipe")}</p> : null}
            {visibleDetail.hasWorkingDraft && !cached ? <Link href={`/my-recipes/${recipeId}/edit`} className="mt-2 inline-flex min-h-11 items-center text-sm font-medium underline underline-offset-4">{nt("continueWorkingDraft")}</Link> : null}
          </div>
          <div className="flex flex-wrap gap-2"><Link href={`/my-recipes/${recipeId}/cook`} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-foreground px-3 text-sm font-medium text-background"><UtensilsCrossed className="h-4 w-4" />{nt("startCooking")}</Link><details className="relative"><summary className="inline-flex min-h-11 cursor-pointer list-none items-center rounded-xl border border-border px-3 text-sm font-medium hover:bg-muted">{nt("addTo")}…</summary><div className="absolute end-0 z-20 mt-1 min-w-44 rounded-xl border border-border bg-popover p-1 shadow-lg"><Link className="flex min-h-11 items-center rounded-lg px-3 text-sm hover:bg-muted" href={`/calories?source=recipe&recipeId=${recipeId}&recipeVersionId=${version.id}`}>{nt("diary")}</Link><Link className="flex min-h-11 items-center rounded-lg px-3 text-sm hover:bg-muted" href={`/my-meal-plan?source=recipe&recipeId=${recipeId}&recipeVersionId=${version.id}`}>{nt("mealPlan")}</Link><Link className="flex min-h-11 items-center rounded-lg px-3 text-sm hover:bg-muted" href={`/calories?source=recipe&recipeId=${recipeId}&recipeVersionId=${version.id}&destination=saved_meal`}>{nt("savedMeal")}</Link></div></details></div>
        </div>
