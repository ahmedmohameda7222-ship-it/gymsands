      ...
      <header className="space-y-4 border-b border-border/70 pb-5">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            {detail.hasWorkingDraft && !cached ? <Link href={`/my-recipes/${recipeId}/edit`} className="mt-2 inline-block text-sm font-medium underline underline-offset-4">{nt("continueWorkingDraft")}</Link> : null}
          </div>
          <div className="flex flex-wrap gap-2"><Link href={`/my-recipes/${recipeId}/cook`} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-foreground px-3 text-sm font-medium text-background"><UtensilsCrossed className="h-4 w-4" />{nt("startCooking")}</Link><details className="relative"><summary className="inline-flex min-h-11 cursor-pointer list-none items-center rounded-xl border border-border px-3 text-sm font-medium hover:bg-muted">{nt("addTo")}…</summary><div className="absolute end-0 z-20 mt-1 min-w-44 rounded-xl border border-border bg-popover p-1 shadow-lg"><Link className="block rounded-lg px-3 py-2 text-sm hover:bg-muted" href={`/calories?source=recipe&recipeId=${recipeId}&recipeVersionId=${version.id}`}>{nt("diary")}</Link><Link className="block rounded-lg px-3 py-2 text-sm hover:bg-muted" href={`/my-meal-plan?source=recipe&recipeId=${recipeId}&recipeVersionId=${version.id}`}>{nt("mealPlan")}</Link><span className="block px-3 py-2 text-sm text-muted-foreground">{nt("savedMeal")}</span></div></details></div>
        </div>
