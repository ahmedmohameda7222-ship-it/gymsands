...
type Ingredient = { food_id: string | null; ingredient_name: string; quantity: string; unit: string; frozen_nutrition: Record<string, unknown> | null; };
type Instruction = { instruction: string; duration_seconds: string; heat_or_temperature: string; doneness_or_result_cue: string; prep_ahead_cue: string; };
...
function initialIngredient(item: Workspace["ingredients"][number]): Ingredient { return { food_id: item.food_id, ingredient_name: item.ingredient_name, quantity: item.quantity === null ? "" : String(item.quantity), unit: item.unit ?? "", frozen_nutrition: item.frozen_nutrition }; }
function initialInstruction(item: Workspace["instructions"][number]): Instruction { return { instruction: item.instruction, duration_seconds: item.duration_seconds === null || item.duration_seconds === undefined ? "" : String(item.duration_seconds), heat_or_temperature: item.heat_or_temperature ?? "", doneness_or_result_cue: item.doneness_or_result_cue ?? "", prep_ahead_cue: item.prep_ahead_cue ?? "" }; }
function initialEquipment(item: Workspace["equipment"][number]): Equipment { return { name: item.name, quantity: item.quantity === null ? "" : String(item.quantity), note: item.note ?? "" }; }
...
export function RecipeEditor({ recipeId, initialAssistantMode, linkedFood }: { recipeId: string; initialAssistantMode?: "create" | "import" | "finish" | null; linkedFood?: { id: string; name: string } | null; }) {
  ...
  const payload = useMemo<DraftPayload>(() => ({
    ...
    ingredients: ingredients.filter((item) => item.ingredient_name.trim()).map((item) => ({ food_id: item.food_id, ingredient_name: item.ingredient_name.trim(), quantity: numberOrNull(item.quantity), unit: textOrNull(item.unit), frozen_nutrition: item.frozen_nutrition })),
    instructions: instructions.filter((item) => item.instruction.trim()).map((item) => ({ instruction: item.instruction.trim(), duration_seconds: numberOrNull(item.duration_seconds), heat_or_temperature: textOrNull(item.heat_or_temperature), doneness_or_result_cue: textOrNull(item.doneness_or_result_cue), prep_ahead_cue: textOrNull(item.prep_ahead_cue) })),
    equipment: equipment.filter((item) => item.name.trim()).map((item) => ({ name: item.name.trim(), quantity: numberOrNull(item.quantity), note: textOrNull(item.note) })),
