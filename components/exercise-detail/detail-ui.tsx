...
export function DetailSurface({ children, className, ariaLabelledby }: { children: ReactNode; className?: string; ariaLabelledby?: string }) {
  return <section aria-labelledby={ariaLabelledby} className={cn("rounded-2xl border bg-card px-4 py-5 sm:px-5 sm:py-6 lg:px-6", className)}>{children}</section>;
}
...
export function ExerciseDetailPageFrame({ children, child, title, description }: {
  ...
  if (state !== "ready" || !resolved) {
    return <TrainPageContainer className="max-w-[1080px] py-4" dir={dir}>
  ...
  return <TrainPageContainer className="max-w-[1080px] pb-24 pt-2 sm:py-5" dir={dir}>
    <nav aria-label={ed("backShort")} className="sticky top-0 z-20 -mx-2 flex min-h-14 items-center justify-between gap-2 bg-background/95 px-2 supports-[backdrop-filter]:bg-background/90 sm:static sm:mx-0 sm:bg-transparent sm:px-0">
