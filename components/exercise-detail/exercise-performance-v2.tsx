...
import { workoutHistoryHrefForExercise, type CanonicalExerciseIdentity } from "@/lib/exercise-detail/identity";
...
export function ExercisePerformancePageContent({ identity }: { identity: CanonicalExerciseIdentity }) {
  ...
  const allSessionsHref = workoutHistoryHrefForExercise(identity);
  return <div className="space-y-5">
    ...
    <DetailSurface ariaLabelledby="performance-recent-sessions"><DetailGroupTitle id="performance-recent-sessions">{ed("recentSessions")}</DetailGroupTitle>{data?.recentSessions.length ? <div className="mt-3 divide-y">{data.recentSessions.map((session) => <Link key={session.activityId} href={`/workout-history/${encodeURIComponent(session.canonicalSessionId ?? session.activityId)}`} className="flex min-h-14 items-center justify-between gap-4 py-3 focus-visible:ring-2 focus-visible:ring-ring"><span><span className="font-medium">{session.title}</span><span className="mt-0.5 block text-sm text-muted-foreground">{formatDate(session.effectiveAt, locale)}</span></span><span aria-hidden="true">›</span></Link>)}</div> : <p className="mt-3 text-sm text-muted-foreground">{ed("noPerformance")}</p>}<div className="mt-3 grid gap-1 border-t pt-2"><DetailRowLink href={allSessionsHref} title={ed("allSessions")} /><DetailRowLink href="/personal-records" title={ed("personalRecords")} /></div></DetailSurface>
  </div>;
}
