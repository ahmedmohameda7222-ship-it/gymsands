...
import { buildOnboardingReviewSections } from "@/lib/onboarding/review-summary";
...
type LocalizedFieldErrors = Record<string, string>;
...
export function AdaptiveOnboardingForm() {
  ...
  const { dir, ot, goalLabel, sportLabel, fieldLabel, optionLabel, dayLabel, permissionLabel, permissionDescription, validationMessage, reviewText } = useOnboardingTranslation();
  const editMode = searchParams.get("edit") === "true";
  const safeReturnTo = resolveOnboardingReturnRoute(searchParams.get("returnTo"));
  ...
  const localizedErrors = useMemo<LocalizedFieldErrors>(() => Object.fromEntries(Object.entries(errors).map(([key, issue]) => [key, validationMessage(issue)])), [errors, validationMessage]);
  const reviewSections = useMemo(() => buildOnboardingReviewSections({
    answers, permissions, weightUnit: settings.weightUnit, heightUnit: settings.heightUnit,
    labels: { text: reviewText, goalLabel, sportLabel, optionLabel, fieldLabel, dayLabel, permissionLabel }
  }), [answers, dayLabel, fieldLabel, goalLabel, optionLabel, permissionLabel, permissions, reviewText, settings.heightUnit, settings.weightUnit, sportLabel]);
  ...
  useEffect(() => {
    if (!dirty || isSaving || allowExitRef.current) return;
    const handler = (event: BeforeUnloadEvent) => { event.preventDefault(); event.returnValue = ""; };
    window.addEventListener("beforeunload", handler);
    return () => window.removeEventListener("beforeunload", handler);
  }, [dirty, isSaving]);
  useEffect(() => {
    if (!editMode || !dirty || isSaving) return;
    const marker = { plaivraOnboardingGuard: true };
    window.history.pushState(marker, "", window.location.href);
    const handlePopState = () => {
      if (allowExitRef.current) return;
      pendingExitRef.current = safeReturnTo;
      setShowCancelConfirm(true);
      window.history.pushState(marker, "", window.location.href);
    };
    window.addEventListener("popstate", handlePopState);
    return () => window.removeEventListener("popstate", handlePopState);
  }, [dirty, editMode, isSaving, safeReturnTo]);
  ...
  function updateGoals(goal: GoalId) {
    ...
    clearErrorKeys("goals", "primary_goal", "goal_weight_kg");
  }
  function updateSport(primarySport: SportId) {
    setAnswers((current) => ({ ...current, primary_sport: primarySport, primary_sport_other: primarySport === "other" ? current.primary_sport_other : "", secondary_sports: current.secondary_sports.filter((sport) => sport !== primarySport) }));
    clearErrorKeys("primary_sport", "primary_sport_other");
  }
  function setSportDetail(id: string, value: string | number | string[] | null) {
    setAnswers((current) => ({ ...current, sport_details: { ...current.sport_details, [id]: value } }));
    clearErrorKeys(`sport_details.${id}`);
  }
  ...
  async function finish() {
    ...
    try {
      ...
      setAnswers(sanitized); setExistingCompletedAt(new Date().toISOString()); setInitialSnapshot(snapshot(sanitized, permissions, permissionConfirmed)); allowExitRef.current = true;
      toast({ title: ot("saved"), description: ot("savedDetail"), variant: "success" }); router.push(editMode ? safeReturnTo : "/dashboard");
    } catch (error) { setSaveError(error instanceof Error ? error.message : ot("saveFailure")); }
    finally { setIsSaving(false); }
  }

  function exitToReturnRoute() {
    allowExitRef.current = true;
    setInitialSnapshot(currentSnapshot);
    router.push(safeReturnTo);
  }
  function requestCancel() {
    if (onboardingExitDecision(dirty) === "exit") { exitToReturnRoute(); return; }
    pendingExitRef.current = safeReturnTo;
    setShowCancelConfirm(true);
  }
  ...
  function selectPermissionMode(mode: "full" | "custom") {
    if (permissionStatus === "failed" || statuses.permissions === "loading") return;
    setPermissions((current) => ({ ...current, accessMode: mode }));
    setPermissionConfirmed(false);
    clearErrorKeys("permissions", "permission_confirmation");
  }
  ...
  return (
    <div dir={dir}>
      <ConfirmDialog open={showCancelConfirm} title={ot("cancelQuestion")} description={ot("cancelDetail")} confirmLabel={ot("discard")} cancelLabel={ot("stay")} variant="destructive" onConfirm={discardAndExit} onCancel={() => { pendingExitRef.current = null; setShowCancelConfirm(false); }} />
      ...
      <div className="mx-auto max-w-5xl space-y-4">
        {Object.entries(sourceErrors).map(([source, message]) => message ? <SourceErrorCard key={source} title={sourceLabel(source as SourceName, ot)} message={message} retryLabel={ot("retry")} onRetry={() => void retrySource(source as SourceName)} loading={statuses[source as SourceName] === "loading"} /> : null)}
          ...
          <CardContent className="space-y-6 p-4 pb-28 sm:p-6 sm:pb-6">
            {Object.keys(errors).length ? <div ref={errorSummaryRef} tabIndex={-1} role="alert" className="rounded-xl border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive outline-none"><p className="font-semibold">{ot("reviewErrors")}</p><ul className="mt-2 list-disc space-y-1 ps-5">{Object.values(localizedErrors).filter(Boolean).map((message, index) => <li key={`${message}-${index}`}>{message}</li>)}</ul></div> : null}
            {saveError ? <PersistentError message={saveError} /> : null}
            {step === 0 ? <BasicProfileSection answers={answers} setAnswers={setAnswers} errors={localizedErrors} ot={ot} weightUnit={settings.weightUnit} heightUnit={settings.heightUnit} /> : null}
            {step === 1 ? <GoalsSection answers={answers} setAnswers={setAnswers} errors={localizedErrors} updateGoals={updateGoals} goalLabel={goalLabel} ot={ot} weightUnit={settings.weightUnit} /> : null}
            {step === 2 ? <TrainingSection answers={answers} setAnswers={setAnswers} errors={localizedErrors} updateSport={updateSport} setSportDetail={setSportDetail} sportLabel={sportLabel} fieldLabel={fieldLabel} optionLabel={optionLabel} dayLabel={dayLabel} ot={ot} /> : null}
            {step === 3 ? <NutritionSection answers={answers} setAnswers={setAnswers} errors={localizedErrors} ot={ot} optionLabel={optionLabel} /> : null}
            {step === 4 ? <ConstraintsSection answers={answers} setAnswers={setAnswers} ot={ot} /> : null}
            {step === 5 ? <PermissionsSection permissions={permissions} permissionConfirmed={permissionConfirmed} permissionStatus={permissionStatus} errors={localizedErrors} selectMode={selectPermissionMode} togglePermission={togglePermission} setPermissionConfirmed={setPermissionConfirmed} retry={() => void retrySource("permissions")} permissionLabel={permissionLabel} permissionDescription={permissionDescription} ot={ot} /> : null}
            {step === 6 ? <section className="space-y-4"><SectionIntro>{ot("reviewIntro")}</SectionIntro><OnboardingReview sections={reviewSections} editLabel={ot("edit")} noValue={ot("noValue")} onEdit={setStep} /></section> : null}
...
function BasicProfileSection({ answers, setAnswers, errors, ot, weightUnit, heightUnit }: { answers: AdaptiveOnboardingAnswers; setAnswers: React.Dispatch<React.SetStateAction<AdaptiveOnboardingAnswers>>; errors: LocalizedFieldErrors; ot: ReturnType<typeof useOnboardingTranslation>["ot"]; weightUnit: "kg" | "lb"; heightUnit: "cm" | "ft-in" }) {
...
function NutritionSection({ answers, setAnswers, errors, ot, optionLabel }: { answers: AdaptiveOnboardingAnswers; setAnswers: React.Dispatch<React.SetStateAction<AdaptiveOnboardingAnswers>>; errors: LocalizedFieldErrors; ot: ReturnType<typeof useOnboardingTranslation>["ot"]; optionLabel: (value: string) => string }) {
...
function PermissionsSection({ permissions, permissionConfirmed, permissionStatus, errors, selectMode, togglePermission, setPermissionConfirmed, retry, permissionLabel, permissionDescription, ot }: { permissions: AiPermissionConfig; permissionConfirmed: boolean; permissionStatus: AiPermissionSettingsStatus["state"] | "loading"; errors: LocalizedFieldErrors; selectMode: (mode: "full" | "custom") => void; togglePermission: (section: (typeof ALL_AI_PERMISSION_SECTIONS)[number], action: "read" | "write") => void; setPermissionConfirmed: (value: boolean) => void; retry: () => void; permissionLabel: (section: (typeof ALL_AI_PERMISSION_SECTIONS)[number]) => string; permissionDescription: (section: (typeof ALL_AI_PERMISSION_SECTIONS)[number], action: "read" | "write") => string; ot: ReturnType<typeof useOnboardingTranslation>["ot"] }) {
