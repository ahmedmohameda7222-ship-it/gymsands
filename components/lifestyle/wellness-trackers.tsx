...
function Field({ label, value, onChange, type = "text", placeholder, inputMode, enterKeyHint }: { label: string; value: string; onChange: (value: string) => void; type?: string; placeholder?: string; inputMode?: React.InputHTMLAttributes<HTMLInputElement>['inputMode']; enterKeyHint?: React.InputHTMLAttributes<HTMLInputElement>['enterKeyHint'] }) { return <div className="space-y-2"><Label>{label}</Label><Input type={type} inputMode={inputMode} enterKeyHint={enterKeyHint} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder ?? label} /></div>; }
