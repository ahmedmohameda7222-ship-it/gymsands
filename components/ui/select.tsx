...
export function SelectContent({
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content className={cn("solid-tracking-card z-50 min-w-40 overflow-hidden", className)}>
...
export function SelectItem({ className, children, ...props }: React.ComponentPropsWithoutRef<typeof SelectPrimitive.Item>) {
  return (
    <SelectPrimitive.Item
      className={cn("relative flex cursor-pointer select-none items-center rounded-xl py-2 pl-8 pr-2 text-sm outline-none focus:bg-muted", className)}
      {...props}
    >
