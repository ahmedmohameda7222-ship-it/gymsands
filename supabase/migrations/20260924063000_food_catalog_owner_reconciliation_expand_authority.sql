...
-- Plan 7 Tasks 13-14: backward-compatible owner authority expand.
-- Repository-only until a fresh Production Personal Correction count satisfies
-- the documented manual apply precondition. No owner rows are migrated here.
...
begin
  if p_user_id is null then
    raise exception 'Personal Override owner ID is required.' using errcode='22023';
  end if;
...
begin
  ...
  return private.food_catalog_get_current_personal_override_for_owner_v1(v_user, p_food_id);
end
$function$;
...
create or replace function private.food_catalog_owner_for_active_mcp_connection_v1(
...
begin
  if p_connection_id is null then
    raise exception 'MCP connection ID is required.' using errcode='42501';
  end if;
  ...
  if not found or v_user_id is null then
    raise exception 'Active MCP connection is required.' using errcode='42501';
  end if;
...
revoke all on function private.food_catalog_owner_for_active_mcp_connection_v1(uuid)
from public, anon, authenticated, service_role;
...
begin
  ...
  v_user_id := private.food_catalog_owner_for_active_mcp_connection_v1(p_connection_id);
...
create or replace function private.food_catalog_personal_override_numeric_v1(
  p_state jsonb,
  p_key text
)
returns numeric
...
begin
  ...
  if p_state is null
     or not coalesce((p_state->>'hasOverride')::boolean,false)
     or coalesce((p_state->>'isDeleted')::boolean,false)
     or jsonb_typeof(p_state->'nutritionOverride') is distinct from 'object' then
    return null;
  end if;

  v_value := p_state->'nutritionOverride'->p_key;
...
revoke all on function private.food_catalog_personal_override_numeric_v1(jsonb,text)
from public, anon, authenticated, service_role;

create or replace function private.food_catalog_personal_override_has_nutrition_v1(
  p_state jsonb
)
returns boolean
language sql
...
as $function$
  select
    private.food_catalog_personal_override_numeric_v1(p_state,'calories') is not null
    or private.food_catalog_personal_override_numeric_v1(p_state,'protein_g') is not null
    or private.food_catalog_personal_override_numeric_v1(p_state,'carbs_g') is not null
    or private.food_catalog_personal_override_numeric_v1(p_state,'fat_g') is not null
    or private.food_catalog_personal_override_numeric_v1(p_state,'saturated_fat_g') is not null
    or private.food_catalog_personal_override_numeric_v1(p_state,'fiber_g') is not null
    or private.food_catalog_personal_override_numeric_v1(p_state,'sugars_g') is not null
    or private.food_catalog_personal_override_numeric_v1(p_state,'sodium_mg') is not null
$function$;

revoke all on function private.food_catalog_personal_override_has_nutrition_v1(jsonb)
from public, anon, authenticated, service_role;

create or replace function private.search_food_catalog_v2_for_owner_v1(
  p_user_id uuid,
...
begin
  if p_user_id is null then
    raise exception 'Trusted Food Catalog owner identity is required.' using errcode = '42501';
  end if;
  ...
  global_scored as (
    select
      ...
      (
        nutrition.id is not null
        and private.food_catalog_personal_override_has_nutrition_v1(override_state.state)
      ) as using_personal_values,
      case
        when nutrition.id is not null
         and private.food_catalog_personal_override_numeric_v1(override_state.state, 'calories') is not null
        then private.food_catalog_search_per_100_v2(
          private.food_catalog_personal_override_numeric_v1(override_state.state, 'calories'),
          nutrition.basis_amount,
          nutrition.basis_unit
        )
        else doc.calories_100
      end as calories_100,
      case
        when nutrition.id is not null
         and private.food_catalog_personal_override_numeric_v1(override_state.state, 'protein_g') is not null
        then private.food_catalog_search_per_100_v2(
          private.food_catalog_personal_override_numeric_v1(override_state.state, 'protein_g'),
          nutrition.basis_amount,
          nutrition.basis_unit
        )
      ...
      case
        when nutrition.id is not null
         and private.food_catalog_personal_override_numeric_v1(override_state.state, 'carbs_g') is not null
        then private.food_catalog_search_per_100_v2(
          private.food_catalog_personal_override_numeric_v1(override_state.state, 'carbs_g'),
          nutrition.basis_amount,
          nutrition.basis_unit
        )
        else doc.carbs_100
      ...
      case
        when nutrition.id is not null
         and private.food_catalog_personal_override_numeric_v1(override_state.state, 'fat_g') is not null
        then private.food_catalog_search_per_100_v2(
          private.food_catalog_personal_override_numeric_v1(override_state.state, 'fat_g'),
          nutrition.basis_amount,
          nutrition.basis_unit
        )
        else doc.fat_100
      ...
      case
        when nutrition.id is not null
         and private.food_catalog_personal_override_numeric_v1(override_state.state, 'saturated_fat_g') is not null
        then private.food_catalog_search_per_100_v2(
          private.food_catalog_personal_override_numeric_v1(override_state.state, 'saturated_fat_g'),
          nutrition.basis_amount,
          nutrition.basis_unit
        )
      ...
      case
        when nutrition.id is not null
         and private.food_catalog_personal_override_numeric_v1(override_state.state, 'fiber_g') is not null
        then private.food_catalog_search_per_100_v2(
          private.food_catalog_personal_override_numeric_v1(override_state.state, 'fiber_g'),
          nutrition.basis_amount,
          nutrition.basis_unit
        )
        else doc.fiber_100
      ...
      case
        when nutrition.id is not null
         and private.food_catalog_personal_override_numeric_v1(override_state.state, 'sugars_g') is not null
        then private.food_catalog_search_per_100_v2(
          private.food_catalog_personal_override_numeric_v1(override_state.state, 'sugars_g'),
          nutrition.basis_amount,
          nutrition.basis_unit
        )
        else doc.sugars_100
      ...
      case
        when nutrition.id is not null
         and private.food_catalog_personal_override_numeric_v1(override_state.state, 'sodium_mg') is not null
        then private.food_catalog_search_per_100_v2(
          private.food_catalog_personal_override_numeric_v1(override_state.state, 'sodium_mg'),
          nutrition.basis_amount,
          nutrition.basis_unit
        )
        else doc.sodium_mg_100
      end as sodium_mg_100,
      doc.nutrition_basis_unit as nutrition_basis_unit,
      case
    ...
    left join lateral (
      select private.food_catalog_get_current_personal_override_for_owner_v1(
        p_user_id,
        doc.food_id
      ) as state
...
revoke all on function private.search_food_catalog_v2_for_owner_v1(uuid,text,text,text,text,text,integer,text,text,text,jsonb)
from public, anon, authenticated, service_role;
...
begin
  ...
  return private.search_food_catalog_v2_for_owner_v1(
    v_user_id,
    p_query,p_language_tag,p_script_code,p_market_scope_code,p_cursor,p_limit,
    p_category,p_cuisine,p_scope,p_filters
  );
end
$function$;
