...
begin
  ...
  if exists (
    select 1
    from jsonb_array_elements(coalesce(p_instructions, '[]'::jsonb)) as action(value)
    cross join lateral jsonb_array_elements_text(coalesce(action.value->'dependency_action_ids', '[]'::jsonb)) as dependency(value)
    where not exists (
      select 1
      from jsonb_array_elements(coalesce(p_instructions, '[]'::jsonb)) as candidate(value)
      where nullif(candidate.value->>'id', '') = dependency.value
    )
  ) then
    raise exception 'Recipe action dependency does not reference an action in the submitted Draft graph.' using errcode = '22023';
  end if;
  ...
  loop
    insert into public.nutrition_recipe_ingredients (
      id, user_id, recipe_version_id, recipe_draft_id, position, food_id, ingredient_name, quantity, unit, frozen_nutrition
    ) values (
  ...
  loop
    insert into public.nutrition_recipe_actions (
      id, user_id, recipe_version_id, recipe_draft_id, position, instruction, ingredient_refs, equipment_refs,
