...
select public.upsert_workout_set_logs_atomic(
  ...
  pg_catalog.current_setting('plaivra.aw3a_session_id')::uuid,
...
begin
  ...
  where workout_session_id=pg_catalog.current_setting('plaivra.aw3a_session_id')::uuid and exercise_order=1 and set_number=1;
...
select pg_catalog.set_config(
  ...
  (select count(*) from public.exercise_logs where workout_session_id=pg_catalog.current_setting('plaivra.aw3a_session_id')::uuid)::text,
  true
);
set local role service_role;
select set_config('request.jwt.claim.role','service_role',true);
...
begin
  ...
  begin
    perform public.upsert_workout_set_logs_atomic(
      'a3000000-0000-4000-8000-000000000001',pg_catalog.current_setting('plaivra.aw3a_session_id')::uuid,
      jsonb_build_array(jsonb_build_object(
...
begin
  if (select count(*) from public.exercise_logs where workout_session_id=pg_catalog.current_setting('plaivra.aw3a_session_id')::uuid)
...
begin
  ...
  select * into strict v_log from public.exercise_logs
  where workout_session_id=pg_catalog.current_setting('plaivra.aw3a_session_id')::uuid and exercise_order=2 and set_number=1;
  ...
  select * into strict v_log from public.exercise_logs
  where workout_session_id=pg_catalog.current_setting('plaivra.aw3a_session_id')::uuid and exercise_order=3 and set_number=1;
...
begin
  if (select count(*) from public.exercise_logs where workout_session_id=pg_catalog.current_setting('plaivra.aw3a_session_id')::uuid)<>1 then
  ...
  if exists (
    ...
    where v.workout_session_id=pg_catalog.current_setting('plaivra.aw3a_session_id')::uuid and l.id is null
  ) then raise exception 'Completion left orphan metric values.'; end if;
  if not exists (
    select 1 from public.workout_session_timeline_events
    where workout_session_id=pg_catalog.current_setting('plaivra.aw3a_session_id')::uuid and event_type='session_completed'
