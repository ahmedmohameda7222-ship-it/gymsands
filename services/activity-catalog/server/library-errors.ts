export type LibraryProviderErrorCode =
  | "catalog_bad_request"
  ...
  | "catalog_forbidden"
  | "catalog_not_found"
  | "catalog_incompatible_cursor"
  | "catalog_upstream_error"
  | "catalog_invalid_response";

const statusByCode: Record<LibraryProviderErrorCode, number> = {
  ...
  catalog_incompatible_cursor: 409,
  catalog_upstream_error: 503,
  catalog_invalid_response: 502
};

const messageByCode: Record<LibraryProviderErrorCode, string> = {
  ...
  catalog_incompatible_cursor: "The exercise catalog page cursor is no longer compatible with the current release.",
...
export class LibraryProviderError extends Error {
  readonly code: LibraryProviderErrorCode;
  readonly status: number;
  readonly allowLegacyFallback: boolean;
  readonly upstreamCode?: string;
  constructor(code: LibraryProviderErrorCode, options: { allowLegacyFallback?: boolean; upstreamCode?: string; cause?: unknown } = {}) {
    super(messageByCode[code], { cause: options.cause });
    this.name = "LibraryProviderError";
    this.code = code;
...
export function asLibraryProviderError(error: unknown) {
  return error instanceof LibraryProviderError ? error : new LibraryProviderError("catalog_upstream_error", { cause: error });
}
