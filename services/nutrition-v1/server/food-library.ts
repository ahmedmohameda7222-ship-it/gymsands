...
function basisUnit(value: unknown): "g" | "ml" | null {
...
function normalizedNutrient(nutrition: FoodLibraryNutrition, key: "protein_g" | "carbs_g" | "fat_g" | "calories") {
  ...
  if (value === null || nutrition.basis_amount === null || nutrition.basis_unit === null || nutrition.basis_amount <= 0) return null;
...
function nutritionFromCatalog(row: Record<string, unknown>): FoodLibraryNutrition {
  return {
    ...
    saturated_fat_g: numeric(row.saturated_fat_g),
    fiber_g: numeric(row.fiber_g),
    sugars_g: numeric(row.sugars_g),
    sodium_mg: numeric(row.sodium_mg),
    basis_amount: numeric(row.nutrition_basis_amount),
...
function parseCorrection(row: Record<string, unknown>): FoodLibraryCorrection {
  return {
    ...
    sugars_g: numeric(row.sugars_g),
    sodium_mg: numeric(row.sodium_mg),
    basis_amount: numeric(row.basis_amount),
    basis_unit: basisUnit(row.basis_unit),
  };
}
...
export async function listFoodLibrary(
  ...
  let catalogQuery = supabase
    .from("food_items")
    .select("id,food_name,serving_size,calories,protein_g,carbs_g,fat_g,category,cuisine,tags,is_verified,saturated_fat_g,fiber_g,sugars_g,sodium_mg,nutrition_basis_amount,nutrition_basis_unit,lifecycle_status")
    .eq("is_global", true)
  ...
  let myFoodQuery = supabase
    .from("user_food_items")
    .select("id,food_name,serving_size,calories,protein_g,carbs_g,fat_g,category,tags,created_at")
    .eq("user_id", userId)
  ...
  const [catalogResult, myFoodResult, aliasResult, favoriteResult, correctionResult, logResult] = await Promise.all([
    ...
    supabase.from("food_personal_corrections")
      .select("food_id,calories,protein_g,carbs_g,fat_g,saturated_fat_g,fiber_g,sugars_g,sodium_mg,basis_amount,basis_unit")
      .eq("user_id", userId).eq("is_active", true).limit(250),
    supabase.from("food_logs").select("food_item_id,user_food_item_id,created_at")
      .eq("user_id", userId).order("created_at", { ascending: false }).limit(250),
  ]);
  ...
  if (missingAliasIds.length) {
    const extra = await supabase
      .from("food_items")
      .select("id,food_name,serving_size,calories,protein_g,carbs_g,fat_g,category,cuisine,tags,is_verified,saturated_fat_g,fiber_g,sugars_g,sodium_mg,nutrition_basis_amount,nutrition_basis_unit,lifecycle_status")
      .in("id", missingAliasIds.slice(0, CANDIDATE_LIMIT))
      .eq("is_global", true)
      .neq("lifecycle_status", "merged");
  ...
  for (const raw of catalogRows) {
    ...
    candidates.push({
      id,
      source: "catalog",
      name: stringOrNull(row.food_name) ?? "Food",
      brand: null,
      category: stringOrNull(row.category),
      cuisine: stringOrNull(row.cuisine),
      servingLabel: stringOrNull(row.serving_size) ?? "Serving",
      verified: row.is_verified === true,
      favorite: favoriteIds.has(id),
      recentAt: use?.recentAt ?? null,
      frequency: use?.frequency ?? 0,
      locale,
      aliases: aliases.get(id) ?? [],
      nutrition: resolveEffectiveFoodNutrition(baseNutrition, correction),
      tags: Array.isArray(row.tags) ? row.tags.filter((tag): tag is string => typeof tag === "string") : [],
      usingPersonalValues: Boolean(correction),
    });
  }
  ...
  for (const raw of myFoodRows) {
    ...
    candidates.push({
      ...
      locale,
      aliases: [],
      nutrition: nutritionFromMyFood(row),
      tags: Array.isArray(row.tags) ? row.tags.filter((tag): tag is string => typeof tag === "string") : [],
      usingPersonalValues: true,
    });
  }
  ...
  return paginateFoodLibraryResults(
    rankFoodLibraryCandidates(filtered, { query, locale }),
