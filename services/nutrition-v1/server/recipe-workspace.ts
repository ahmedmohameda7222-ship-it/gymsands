...
export async function duplicatePublishedRecipe(supabase: SupabaseClient, userId: string, recipeId: string) {
  ...
  const root = requiredData(rootResult.data as { id: string } | null, rootResult.error, "Duplicate Recipe could not be created.");
  ...
  let draft: { id: string };
  try {
    draft = requiredData(draftResult.data as { id: string } | null, draftResult.error, "Duplicate Recipe Draft could not be created.");
  } catch (error) {
  ...
  return { recipeId: root.id, draftId: draft.id };
}
