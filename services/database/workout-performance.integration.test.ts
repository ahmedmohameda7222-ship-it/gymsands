...
const mcpImplementation = readFileSync(
...
describe("AW-3A set-write integration contract", () => {
  it("keeps legacy callers compatible while serializing structured values only when supplied", () => {
    ...
    expect(legacyImplementation).toContain(
      "serializeWorkoutSetLogs(finalLogs)",
    );
  ...
  it("routes browser and MCP set mutations through the canonical atomic RPC", () => {
    ...
    expect(mcpImplementation).toContain(
      '.rpc("upsert_workout_set_logs_atomic"',
    );
    expect(mcpExecutor).toContain(
      'functionName === "upsert_workout_set_logs_atomic"',
    );
    for (const source of [
      writeService,
      legacyService,
  ...
  it("keeps completion on the same public RPC and relies on metric cascade for omitted logs", () => {
    expect(legacyImplementation).toMatch(
      /\.rpc\(\s*"complete_workout_session_atomic"/,
    );
    expect(legacyImplementation).toContain(
      "serializeWorkoutSetLogs(finalLogs)",
    );
