...
function requireWorkoutPersistence(
  userIdOrSessionId: string | null | undefined,
  label: string,
) {
  if (!supabase || !isUuid(userIdOrSessionId)) {
    throw new Error(
      `${label} could not be saved. Please refresh, sign in again, and try once more.`,
    );
  }
}

type WorkoutSessionIdentity = Pick<
  WorkoutSession,
  "id" | "user_id" | "plan_day_id" | "status"
>;

async function getWorkoutSessionIdentity(
  sessionId: string,
): Promise<WorkoutSessionIdentity> {
...
function isMissingTemplateSchemaError(
  error: { message?: string; code?: string } | null | undefined,
) {
  const message = error?.message?.toLowerCase() ?? "";
  return (
...
function normalizeWorkoutSession(session: WorkoutSession): WorkoutSession {
  if (
    ...
    session.notes?.startsWith(skippedNotePrefix)
  ) {
    return {
      ...session,
      status: "skipped",
...
async function attachLegacyCatalogIdentity(sessions: WorkoutSessionSummary[]) {
  const planExerciseIds = Array.from(
    new Set(
      sessions
        .flatMap((session) => session.exercise_logs ?? [])
  ...
  if (error) {
    console.warn(
      "Plaivra could not attach stable exercise identities to workout history.",
      error.message,
    );
    return sessions;
  }

  const sourceByPlanExercise = new Map(
    (data ?? []).map((row) => [
      row.id,
      row.source_workout_id ?? row.workout_id ?? null,
    ]),
  );
  return sessions.map((session) => ({
    ...
    exercise_logs: (session.exercise_logs ?? []).map((log) => ({
      ...log,
      source_workout_id: log.plan_exercise_id
        ? (sourceByPlanExercise.get(log.plan_exercise_id) ?? null)
        : null,
    })),
  }));
}
...
function scheduledSessionDate(session: UserWorkoutSession) {
  return (
    ...
    session.skipped_at ||
    session.started_at ||
    `${session.scheduled_date}T00:00:00.000Z`
  );
}

function mapScheduledSessionToWorkoutSession(
...
function normalizeScheduledSession(
  ...
  return {
    ...
    duration_minutes: session.duration_minutes,
    notes: session.notes,
    logs: [...(session.user_exercise_logs ?? [])].sort(
...
export async function startWorkoutDaySession(
  ...
  if (error) {
    console.warn(
      "Plaivra could not start a workout day session.",
      error.message,
    );
    throw error;
  }
  ...
  if (!result?.session)
    throw new Error("Workout session could not be started.");
  return normalizeWorkoutSession(result.session);
}

export async function getOpenWorkoutDaySession(
  userId: string,
  planDayId: string,
) {
  ...
  if (error) {
    console.warn(
      "Plaivra could not load the open workout session.",
      error.message,
    );
    return null;
  }
...
export async function getOrStartWorkoutDaySession(
  userId: string,
  day: WorkoutPlanDaySession,
) {
...
export async function getWorkoutSessionLogs(sessionId: string) {
  ...
  const { data, error } = await supabase!
    .from("exercise_logs")
    .select(
      "*,set_details:exercise_log_set_details(*),segments:exercise_log_set_segments(*,metric_values:exercise_log_set_segment_metric_values(*))",
    )
...
export async function completeWorkoutSession(
  ...
  if (error) {
    console.warn(
      "Plaivra could not complete this workout session.",
      error.message,
    );
    throw error;
  }
...
export async function replaceWorkoutSessionExercise(
  ...
  const replacementType =
    replacement.catalog_source === "custom"
      ? "custom_exercise"
  ...
  if (error) {
    console.warn(
      "Plaivra could not record the stable replacement identity.",
      error.message,
    );
    throw error;
  }
...
export async function getOpenWorkoutSession(
  ...
  if (error) {
    console.warn(
      "Plaivra could not load the active workout session.",
      error.message,
    );
    return null;
  }
...
export async function getOpenWorkoutSessionWithStatus(
  ...
  if (env.useMockAuth && isMockAuthUserId(userId) && !env.productionQaBuild) {
    const session =
      getMockTrainActivity().find(
        (item) =>
          item.status === "started" &&
          (!workoutId || item.workout_id === workoutId),
      ) ?? null;
    return { session };
  }
  ...
  if (candidateSessionId && isUuid(candidateSessionId)) {
    ...
    if (candidate.error) {
      console.warn(
        "Plaivra could not validate the active workout session.",
    ...
    if (candidate.data)
      return {
        session: normalizeWorkoutSession(candidate.data as WorkoutSession),
      };
  }
  let query = supabase!
    .from("workout_sessions")
  ...
  return {
    session: data ? normalizeWorkoutSession(data as WorkoutSession) : null,
  };
}
...
export async function getWorkoutHistoryDetailed(userId: string, limit = 100) {
  ...
  if (error) {
    console.warn(
      "Plaivra could not load workout history details.",
      error.message,
    );
    return [];
  }
  const normalized = ((data ?? []) as WorkoutSessionSummary[])
    .map((session) => ({
      ...
      exercise_logs: sortExerciseLogsByWorkoutOrder(
...
export async function getWorkoutHistoryDetailedWithStatus(
  userId: string,
  limit = 100,
): Promise<WorkoutHistorySourceResult<WorkoutSessionSummary[]>> {
  if (!canUseUserData(userId)) {
    return {
      data: [],
      status: {
        ...
        state: "failed",
        message:
          "Completed workout sessions could not load because the user session is invalid.",
      },
    };
  }
...
export async function getScheduledWorkoutHistory(userId: string, limit = 100) {
  ...
  if (error) {
    if (!isMissingTemplateSchemaError(error))
      console.warn("Plaivra could not load workout history.", error.message);
    return [];
  }

  return ((data ?? []) as unknown as RawScheduledSession[]).map(
    normalizeScheduledSession,
  );
}

export async function getScheduledWorkoutHistoryWithStatus(
  userId: string,
  limit = 100,
): Promise<WorkoutHistorySourceResult<UserWorkoutSession[]>> {
  if (!canUseUserData(userId)) {
    return {
      data: [],
      status: {
        ...
        state: "failed",
        message:
          "Imported-plan workout history could not load because the user session is invalid.",
      },
    };
  }
  ...
  return {
    data: ((data ?? []) as unknown as RawScheduledSession[]).map(
      normalizeScheduledSession,
    ),
...
export async function getScheduledWorkoutActivity(userId: string, limit = 180) {
  ...
  const { data, error } = await supabase!
    .from("user_workout_sessions")
    .select(
      "id,user_id,user_workout_plan_id,plan_day_id,week_index,day_index,session_number,scheduled_date,day_title,status,started_at,completed_at,skipped_at,duration_minutes,notes",
    )
    .eq("user_id", userId)
    .in("status", ["completed", "skipped"])
    .order("scheduled_date", { ascending: false })
  ...
  return ((data ?? []) as unknown as RawScheduledSession[]).map((session) =>
    mapScheduledSessionToWorkoutSession(normalizeScheduledSession(session)),
  );
}
