...
function main() {
  ...
  const headReportPath = join(resolvedOutputDir, "aw2a-head-unit.json");
  const baseReportPath = join(resolvedOutputDir, "aw2a-base-unit.json");
  const parityReportPath = join(resolvedOutputDir, "aw2a-unit-failure-parity.json");
  const baseWorktreeRoot = mkdtempSync(join(tmpdir(), "plaivra-aw2a-base-"));
