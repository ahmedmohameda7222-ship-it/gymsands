...
function qid(value) { return `"${assertIdentifier(value)}"`; }
...
function validateSourceValueTransitions(spec, stableKey) {
  for (const transition of spec.sourceValueTransitions ?? []) {
    if (!transition || typeof transition !== "object") throw new Error(`Malformed source value transition for ${spec.relation}.`);
    const column = assertIdentifier(transition.column);
    if (stableKey.includes(column)) throw new Error(`Source value transition ${spec.relation}.${column} cannot change a stable key.`);
    if (typeof transition.from !== "string" || typeof transition.to !== "string") {
      throw new Error(`Source value transition ${spec.relation}.${column} requires string from/to values.`);
    }
  }
}
...
export function buildSingleSnapshotPsqlProgram({ profile, relations }) {
  ...
  exportable.forEach((spec) => {
    ...
    const stableKey = spec.stableKey.map(assertIdentifier);
    for (const column of (spec.sourceTransientNeutralize ?? []).map(assertIdentifier)) {
      if (stableKey.includes(column)) throw new Error(`Source-transient field ${spec.relation}.${column} cannot be part of the stable key.`);
    }
...
function snapshotBoundarySha(boundary) {
  return sha256(JSON.stringify({
    ...
    migrationCount: boundary.migrationCount, latestMigration: boundary.latestMigration, migrationLedgerIdentity: boundary.migrationLedgerIdentity,
    currentGenerationId: boundary.currentGenerationId, pointerRevision: boundary.pointerRevision,
    compatibilityVersion: boundary.compatibilityVersion, compatibilityMarker: boundary.compatibilityMarker,
  }));
}
function semanticRoot(manifest) {
  return sha256(JSON.stringify({
    format: manifest.format, formatVersion: manifest.formatVersion, canonicalizationVersion: manifest.canonicalizationVersion,
    profile: manifest.profile, registryAuthority: manifest.registryAuthority ?? null, sourceRepositoryCommit: manifest.sourceRepositoryCommit,
    sourceSchemaFingerprintSha256: manifest.sourceSchemaFingerprintSha256,
    snapshotBoundary: { ...manifest.snapshotBoundary },
    segments: [...manifest.segments].map((segment) => ({
      name: segment.name, relation: segment.relation, classification: segment.classification, loadMode: segment.loadMode,
      stableKey: [...segment.stableKey], rowCount: segment.rowCount, plaintextSemanticSha256: segment.plaintextSemanticSha256,
      snapshotBoundarySha256: segment.snapshotBoundarySha256, required: segment.required, protected: segment.protected,
    })).sort((left, right) => compareCodeUnits(left.name, right.name)),
  }));
}

async function loadRuntimeCanonicalizer() {
  return import(pathToFileURL(resolve("lib/food-catalog/portability/canonicalize.ts")).href);
}
async function loadDefaultRules(profile) {
  const { FOOD_CATALOG_PORTABLE_RELATIONS_V1 } = await import(pathToFileURL(resolve("lib/food-catalog/portability/relation-registry.ts")).href);
  return FOOD_CATALOG_PORTABLE_RELATIONS_V1.filter((entry) => entry.requiredProfile === "CORE_PORTABLE" || profile === "FULL_DR");
}
function canonicalRuleIdentity(rule) {
  return JSON.stringify({
    segment: rule.segment ?? rule.relation, relation: rule.relation, classification: rule.classification, loadMode: rule.loadMode,
    stableKey: [...rule.stableKey], requiredProfile: rule.requiredProfile, protected: Boolean(rule.protected),
    sourceTransientNeutralize: [...(rule.sourceTransientNeutralize ?? [])],
    sourceValueTransitions: (rule.sourceValueTransitions ?? []).map((entry) => ({ column: entry.column, from: entry.from, to: entry.to })),
  });
}

export function prepareSourcePortableValues(values, rule) {
  if (!values || typeof values !== "object" || Array.isArray(values)) throw new Error(`Malformed source export values for ${rule?.relation ?? "unknown relation"}.`);
  const sourceTransient = rule?.sourceTransientNeutralize ?? [];
  const transitions = rule?.sourceValueTransitions ?? [];
  if (sourceTransient.length === 0 && transitions.length === 0) return values;
  const stableKey = new Set(rule?.stableKey ?? []);
  const prepared = { ...values };
  for (const column of sourceTransient) {
    if (stableKey.has(column)) throw new Error(`Source-transient field ${rule.relation}.${column} cannot be part of the stable key.`);
    const scalar = values[column];
    if (!scalar || typeof scalar !== "object" || !Object.hasOwn(scalar, "text")) throw new Error(`Declared source-transient field ${rule.relation}.${column} is missing from the export row.`);
    prepared[column] = { ...scalar, text: null };
  }
  for (const transition of transitions) {
    const column = assertIdentifier(transition.column);
    if (stableKey.has(column)) throw new Error(`Source value transition ${rule.relation}.${column} cannot change a stable key.`);
    const scalar = values[column];
    if (!scalar || typeof scalar !== "object" || !Object.hasOwn(scalar, "text")) throw new Error(`Declared source value transition ${rule.relation}.${column} is missing from the export row.`);
    if (scalar.text === transition.from) prepared[column] = { ...scalar, text: transition.to };
  }
  return prepared;
}
...
async function assertRegistryAuthority(profile, rules, registryAuthority) {
  ...
  for (let index = 0; index < canonical.length; index += 1) {
    if (canonicalRuleIdentity(canonical[index]) !== canonicalRuleIdentity(rules[index])) throw new Error(`Canonical ${profile} relation descriptor mismatch at ${canonical[index].segment}.`);
  }
}
async function loadProtectedRuntime() {
  const [keyProvider, protectedSegments] = await Promise.all([
    import(pathToFileURL(resolve("lib/food-catalog/portability/key-provider.ts")).href),
    import(pathToFileURL(resolve("lib/food-catalog/portability/protected-segments.ts")).href),
  ]);
  return {
...
async function closeStream(stream) { stream.end(); await once(stream, "close"); }
async function protectedTransportSha256(path, nonce, authTag) {
  const hash = createHash("sha256");
  hash.update(PROTECTED_TRANSPORT_PREFIX); hash.update(nonce); hash.update(authTag);
  for await (const chunk of createReadStream(path)) hash.update(chunk);
  return hash.digest("hex");
}

export async function runAuthoritativeExport({ databaseUrl, outputDir, profile, sourceRepositoryCommit, rules, registryAuthority = DIAGNOSTIC_REGISTRY_AUTHORITY, protectedKeyProvider = null, protectedKeyId = null }) {
  ...
  if (protectedRules.length > 0 && profile !== "FULL_DR") throw new Error("Protected relations are allowed only in FULL_DR exports.");
  let protectedKey = null; let nonceGuard = null;
  if (protectedRules.length > 0) {
    if (!protectedKeyProvider || typeof protectedKeyId !== "string" || protectedKeyId.trim().length === 0) throw new Error("FULL_DR protected relations require an external protected key provider and key ID; plaintext fallback is forbidden.");
  ...
  child.stdin.end(buildSingleSnapshotPsqlProgram({ profile, relations: rules }), "utf8");
  let stderr = ""; child.stderr.setEncoding("utf8"); child.stderr.on("data", (chunk) => { if (stderr.length < 1_000_000) stderr += chunk; });
  let meta = null; let active = null; const completed = new Map();
  const rulesBySegment = new Map(rules.map((rule) => [rule.segment ?? rule.relation, rule]));
  const lines = createInterface({ input: child.stdout, crlfDelay: Infinity });
  try {
    for await (const line of lines) {
      if (line.startsWith("__PLAN7_META__")) { if (meta) throw new Error("Duplicate Plan7 snapshot metadata record."); meta = JSON.parse(line.slice("__PLAN7_META__".length)); continue; }
      if (line.startsWith("__PLAN7_SEGMENT_BEGIN__")) {
        if (active) throw new Error(`Nested Plan7 segment stream at ${line}.`);
        const name = line.slice("__PLAN7_SEGMENT_BEGIN__".length); const rule = rulesBySegment.get(name);
        if (!rule) throw new Error(`Unexpected Plan7 segment ${name}.`);
        const protectedSegment = Boolean(rule.protected); const path = resolve(staging, "segments", `${name}${protectedSegment ? ".enc" : ".ndjson"}`);
        ...
        active = { name, rule, path, stream: createWriteStream(path, { flags: "wx" }), cipher: protectedSegment ? createCipheriv(PROTECTED_NODE_ALGORITHM, Buffer.from(protectedKey), nonce) : null, nonce, hash: createHash("sha256"), rowCount: 0, previousKey: undefined };
        continue;
      }
      if (line.startsWith("__PLAN7_SEGMENT_END__")) {
        const name = line.slice("__PLAN7_SEGMENT_END__".length); if (!active || active.name !== name) throw new Error(`Unbalanced Plan7 segment end ${name}.`);
        let ciphertextTransportSha256; let encryption;
        if (active.rule.protected) {
          await writeChunk(active.stream, active.cipher.final()); const authTag = active.cipher.getAuthTag(); await closeStream(active.stream);
          ciphertextTransportSha256 = await protectedTransportSha256(active.path, active.nonce, authTag);
          encryption = { algorithm: PROTECTED_ALGORITHM, keyId: protectedKeyId, nonceBase64: active.nonce.toString("base64"), authTagBase64: authTag.toString("base64") };
        } else await closeStream(active.stream);
        completed.set(name, { rowCount: active.rowCount, plaintextSemanticSha256: active.hash.digest("hex"), ciphertextTransportSha256, encryption }); active = null; continue;
      }
      if (!active || line.length === 0) continue;
      if (!/^[0-9a-f]+$/.test(line) || line.length % 2 !== 0) throw new Error(`Malformed hexadecimal COPY transport for ${active.name}.`);
    ...
    if (active) throw new Error(`Plan7 segment ${active.name} did not terminate.`); if (!meta) throw new Error("Plan7 snapshot metadata was not emitted by PostgreSQL.");
    const boundary = { environment: meta.environment, postgresSnapshot: meta.postgresSnapshot, capturedAt: meta.capturedAt, migrationCount: meta.migrationCount, latestMigration: meta.latestMigration, migrationLedgerIdentity: sha256(meta.migrationLedgerIdentityInput ?? ""), currentGenerationId: meta.currentGenerationId ?? null, pointerRevision: meta.pointerRevision, compatibilityVersion: meta.compatibilityVersion, compatibilityMarker: meta.compatibilityMarker, sha256: "" };
    boundary.sha256 = snapshotBoundarySha(boundary);
    const segments = rules.map((rule) => {
      const result = rule.loadMode === "DERIVED_REBUILD" ? { rowCount: 0, plaintextSemanticSha256: EMPTY_SHA256 } : completed.get(rule.segment ?? rule.relation);
      if (!result) throw new Error(`Required Plan7 segment ${rule.segment ?? rule.relation} was not exported.`);
      return { name: rule.segment ?? rule.relation, relation: rule.relation, classification: rule.classification, loadMode: rule.loadMode, stableKey: [...rule.stableKey], rowCount: result.rowCount, plaintextSemanticSha256: result.plaintextSemanticSha256, snapshotBoundarySha256: boundary.sha256, required: true, protected: Boolean(rule.protected), ...(rule.protected ? { ciphertextTransportSha256: result.ciphertextTransportSha256, encryption: result.encryption } : {}) };
    });
    const manifest = { format: "plaivra-food-catalog-portable-export", formatVersion: 1, canonicalizationVersion: 1, profile, registryAuthority, sourceRepositoryCommit, sourceSchemaFingerprintSha256: sha256(meta.schemaIdentityInput ?? ""), capturedAt: meta.capturedAt, snapshotBoundary: boundary, segments, semanticRootSha256: "", certification: { artifactValid: false, restoreVerified: false, drReady: false } };
    manifest.semanticRootSha256 = semanticRoot(manifest);
    await writeFile(resolve(staging, "manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8"); await rm(target, { recursive: true, force: true }); await rename(staging, target);
    if (protectedKey) protectedKey.fill(0); return manifest;
  } catch (error) { child.kill("SIGTERM"); if (protectedKey) protectedKey.fill(0); await rm(staging, { recursive: true, force: true }); throw error; }
}
...
async function main() {
  const options = parseArgs(process.argv.slice(2)); let rules; let registryAuthority;
  if (options.relationsJson) { rules = JSON.parse(await readFile(resolve(options.relationsJson), "utf8")); registryAuthority = DIAGNOSTIC_REGISTRY_AUTHORITY; }
  else { rules = await loadDefaultRules(options.profile); registryAuthority = CANONICAL_REGISTRY_AUTHORITY; }
  const needsProtected = rules.some((rule) => rule.protected); let protectedBinding = null;
  if (needsProtected) { const runtime = await loadProtectedRuntime(); protectedBinding = runtime.createEnvironmentProtectedSegmentKeyBinding(process.env); }
  const manifest = await runAuthoritativeExport({ databaseUrl: process.env.PLAN7_DATABASE_URL, outputDir: options.outputDir, profile: options.profile, sourceRepositoryCommit: options.sourceRepositoryCommit, rules, registryAuthority, protectedKeyProvider: protectedBinding?.keyProvider ?? null, protectedKeyId: protectedBinding?.keyId ?? null });
