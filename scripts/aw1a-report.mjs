...
const unitBaselineStatus = process.env.UNIT_BASELINE_STATUS || "unknown";
...
const fullUnitResult = unitStatus === "success"
  ? "passed"
  : unitBaselineStatus === "success"
    ? "failed with the same four pre-existing assertions as starting main; no AW-1A-specific unit regression was introduced"
    : "failed and baseline parity was not established";
...
- Implementation validation workflow run ID: \`${runId}\`
...
- \`npm run test:unit\`: ${fullUnitResult}.
- Starting-main failure-parity comparison: ${unitBaselineStatus}.
...
Detailed logs and QA evidence are in workflow artifact \`aw1a-validation-${runId}\`.
...
None. Starting \`main\` matched \`${startingSha}\`.
