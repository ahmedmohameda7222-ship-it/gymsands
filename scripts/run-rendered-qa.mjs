...
const evidenceDir = path.resolve(
  process.env.QA_EVIDENCE_DIR ||
    path.join(process.cwd(), "quality-reports", "rendered-qa-evidence"),
);
...
async function createDeterministicContext(browser, viewport) {
  ...
  await context.route("**/api/nutrition/v1/diary*", async (route) => {
    ...
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      headers: { "x-plaivra-qa-fixture": "nutrition-diary-v1" },
      body: JSON.stringify({
        date,
        position: {
          actual,
...
for (
  ...
  for (let routeIndex = 0; routeIndex < routes.length; routeIndex += 1) {
    ...
    const onPageError = (error) =>
      pageErrors.push({
        message: sanitizedText(error.message),
        stack: sanitizedText(error.stack || error.message),
      });
    const onConsole = (message) => {
      if (message.type() === "error") {
        consoleErrors.push({
          category: message.type(),
          message: sanitizedText(message.text(), 800),
        });
      }
    ...
    const metrics = await page.evaluate(() => {
      ...
      const interactive = [
        ...document.querySelectorAll(
          "button, input, select, textarea, [role='button'], nav a",
        ),
      ].filter(visible);
      ...
      const compactTargetDetails = interactive
        .filter((element) => {
          const target =
            element instanceof HTMLInputElement &&
            ["checkbox", "radio"].includes(element.type) &&
      ...
      return {
        ...
        hasRetryControl: Boolean(
          [...document.querySelectorAll("button, a")].find((element) =>
            /retry|try again/i.test(element.textContent || ""),
          ),
        ),
      };
    });
    ...
    const filteredConsoleErrors = consoleErrors.filter(
      (entry) => !/favicon|Failed to load resource.*404/i.test(entry.message),
    );
    const unexpectedConsoleErrors = filteredConsoleErrors;
    const unexpectedConsoleWarnings = consoleWarnings;
    const failed =
      response?.status() !== 200 ||
      metrics.horizontalOverflowPx > 1 ||
      metrics.unnamedInteractiveElements > 0 ||
      pageErrors.length > 0 ||
    ...
    if (failed) {
      ...
      await page.screenshot({
        path: path.join(evidenceDir, failureScreenshot),
        fullPage: true,
      });
    } else if (routeIndex % viewports.length === viewportIndex) {
      await page.screenshot({
        path: path.join(evidenceDir, `${route.name}-${viewport.name}.png`),
        fullPage: true,
      });
    }
    results.push({
...
const navigationContext = await createDeterministicContext(browser, {
  width: 390,
  height: 844,
});
...
navigationPage.on("pageerror", (error) =>
  navigationPageErrors.push({
    message: sanitizedText(error.message),
    stack: sanitizedText(error.stack || error.message),
  }),
);
navigationPage.on("framenavigated", (frame) => {
  if (frame === navigationPage.mainFrame())
    navigationTransitions.push(new URL(frame.url()).pathname);
});
await navigationPage.goto(`${baseUrl}/dashboard`, {
  waitUntil: "networkidle",
  timeout: 30_000,
});
const navigationResponse = await navigationPage.goto(
  `${baseUrl}/today-workout`,
...
const navigationEvidence = {
  ...
  passed:
    ...
    redirectFinalPath === "/my-workout/plans" &&
    awayFinalPath === "/dashboard" &&
    navigationPageErrors.length === 0,
};
if (!navigationEvidence.passed) {
  navigationEvidence.failureScreenshot =
    "failure-today-workout-reused-navigation-390x844.png";
  await navigationPage.screenshot({
    path: path.join(evidenceDir, navigationEvidence.failureScreenshot),
    fullPage: true,
  });
}
...
await zoomPage.screenshot({
  path: path.join(evidenceDir, "landing-390x844-200-percent-text.png"),
  fullPage: true,
});
await zoomPage.evaluate(() =>
  localStorage.setItem("plaivra.language.v1", "ar"),
);
await zoomPage.reload({ waitUntil: "networkidle" });
const localizedOverflow = await zoomPage.evaluate(() =>
  Math.max(
    0,
    document.documentElement.scrollWidth - document.documentElement.clientWidth,
  ),
...
await zoomPage.screenshot({
  path: path.join(evidenceDir, "landing-390x844-arabic.png"),
  fullPage: true,
});
...
const failures = results.filter(
  (item) =>
    ...
    item.unnamedInteractiveElements > 0 ||
...
const report = {
  ...
  productionAcceptanceEvidence: serverMode === "production",
  viewports,
...
await writeFile(
  path.join(evidenceDir, "rendered-qa-results.json"),
