...
const EXPECTED_CATALOG_DEPLOYMENT_ID = "dpl_DUi8u5A82tWGUQMhbV4esjG4N5EW";
const CATALOG_PREVIEW_ORIGIN = "https://plaivra-activity-catalog-293ri9r9k-ahmed-s-projectssasa.vercel.app";
...
const EXPECTED_DOMAINS = [
  "calisthenics", "cardio_conditioning", "cycling", "general_home", "mobility",
  "pilates", "running", "strength", "swimming", "yoga"
];
...
assert(sha40.test(mainHead), "P10F Main Preview exact commit SHA is unavailable.");
...
function assertNoSensitive(value) {
  ...
  for (const token of [apiKey.toLowerCase(), "service_role", "supabase_secret_key", "sqlstate", "postgres://", "postgresql://", "stacktrace"]) {
    if (token) assert(!text.includes(token), `Catalog Preview leaked sensitive/internal material: ${token.slice(0, 12)}`);
  }
}
...
function assertLibraryAuthority(payload) {
  ...
  assert(meta.libraryRelease?.strengthSemanticFingerprint === EXPECTED_STRENGTH_FINGERPRINT, "Catalog Preview Strength fingerprint drifted.");
...
async function requestJson(pathname, { authenticated = true, bearer = apiKey } = {}) {
  const url = new URL(pathname, CATALOG_PREVIEW_ORIGIN);
  for (const key of url.searchParams.keys()) {
    assert(!/user|email|account|profile|session|cookie/i.test(key), `User-owned query parameter must not be sent upstream: ${key}`);
  }
  const headers = { Accept: "application/json" };
  if (authenticated) headers.Authorization = `Bearer ${bearer}`;
  assert(Object.keys(headers).every((key) => key === "Accept" || key === "Authorization"), "Unexpected upstream request header detected.");
  const response = await fetch(url, { method: "GET", cache: "no-store", signal: AbortSignal.timeout(15_000), headers });
...
function proveMainServerBoundary() {
  const runtimeEnv = fs.readFileSync("lib/integrations/env.ts", "utf8");
  assert(runtimeEnv.includes(CATALOG_PREVIEW_ORIGIN), "Main Preview runtime is not pinned to the exact Catalog Preview origin.");
  assert(runtimeEnv.includes("library_v2_with_legacy_fallback"), "Main Preview provider mode is not the approved explicit fallback mode.");
  assert(runtimeEnv.includes("process.env.PLAIVRA_ACTIVITY_CATALOG_API_KEY"), "Main server boundary does not consume the server-only Catalog key.");
  for (const file of ["app", "components", "hooks"].flatMap(collectFiles)) {
...
proveMainServerBoundary();

const unauth = await requestJson("/v2/health", { authenticated: false });
assert(unauth.response.status === 401 && unauth.payload?.error?.code === "unauthorized", "Unauthenticated Catalog health must fail closed with unauthorized/401.");
const invalidAuth = await requestJson("/v2/health", { bearer: "p10f-intentionally-invalid-preview-key" });
assert(invalidAuth.response.status === 401 && invalidAuth.payload?.error?.code === "unauthorized", "Invalid Catalog bearer credential must fail closed with unauthorized/401.");
...
assert(domains.response.ok, `Library domains returned HTTP ${domains.response.status}.`);
...
const page1 = await requestJson("/v2/library-domains/strength/activities?locale=en&limit=20");
assert(page1.response.ok && page1.payload?.data?.length === 20, "Strength first page is invalid.");
assertLibraryAuthority(page1.payload);
const page1Identities = new Set(page1.payload.data.map((item) => `${item.id}:${item.revisionId}`));
assert(page1Identities.size === page1.payload.data.length, "Duplicate identities exist inside the Strength page.");
const cursor = page1.payload?.pagination?.nextCursor;
assert(typeof cursor === "string" && cursor.length > 20, "Strength opaque cursor is missing.");
const page2 = await requestJson(`/v2/library-domains/strength/activities?locale=en&limit=20&cursor=${encodeURIComponent(cursor)}`);
assert(page2.response.ok && page2.payload?.data?.length > 0, "Strength cursor continuation failed.");
assertLibraryAuthority(page2.payload);
for (const item of page2.payload.data) assert(!page1Identities.has(`${item.id}:${item.revisionId}`), "Cursor continuation repeated an identity.");
...
for (const identifier of [candidate.slug, candidate.id, candidate.revisionId]) {
  ...
  assert(detail.response.ok, `Strength detail failed for ${identifier}.`);
  assertLibraryAuthority(detail.payload);
  assert(detail.payload?.data?.id === candidate.id && detail.payload?.data?.revisionId === candidate.revisionId && detail.payload?.data?.slug === candidate.slug, `Stable identifier resolution drifted for ${identifier}.`);
}
...
for (const locale of ["en", "de", "ar"]) {
  ...
  assert(detail.response.ok, `${locale} Strength detail failed.`);
  assertLibraryAuthority(detail.payload);
...
for (const locale of ["de", "ar"]) {
  const search = await requestJson(`/v2/library-domains/strength/activities?locale=${locale}&query=${encodeURIComponent(known[locale].name)}&limit=10`);
  assert(search.response.ok && search.payload.data.some((item) => item.slug === KNOWN_STRENGTH_SLUG), `${locale} localized search missed the known Strength identity.`);
  assertLibraryAuthority(search.payload);
}

const aliasCandidate = [...page1.payload.data, ...page2.payload.data].find((item) => Array.isArray(item.aliases) && item.aliases.some((alias) => typeof alias?.text === "string" && alias.text.trim()));
assert(aliasCandidate, "No representative published alias was available for live acceptance.");
const aliasText = aliasCandidate.aliases.find((alias) => typeof alias?.text === "string" && alias.text.trim()).text;
const aliasSearch = await requestJson(`/v2/library-domains/strength/activities?locale=en&query=${encodeURIComponent(aliasText)}&limit=20`);
assert(aliasSearch.response.ok && aliasSearch.payload.data.some((item) => item.id === aliasCandidate.id), "Representative alias search did not resolve its release-bound identity.");

const zero = await requestJson("/v2/library-domains/strength/activities?locale=en&query=zzzz_p10f_no_match_zzzz&limit=10");
assert(zero.response.ok && Array.isArray(zero.payload?.data) && zero.payload.data.length === 0, "Legitimate zero-result search must remain a successful empty result.");

async function findAlternativeCase(domain, items, predicate) {
  for (const item of items) {
    const result = await requestJson(`/v2/library-domains/${domain}/activities/${item.revisionId}/alternatives?locale=en&limit=6`);
    assert(result.response.ok && Array.isArray(result.payload?.data), `Alternatives failed for ${domain}/${item.slug}.`);
    assertLibraryAuthority(result.payload);
    if (predicate(result.payload.data)) return { item, result };
  }
  return null;
}

const nonEmptyAlternative = await findAlternativeCase("strength", [...page1.payload.data, ...page2.payload.data], (data) => data.length > 0);
assert(nonEmptyAlternative, "No representative non-empty authoritative Strength alternative set was proven.");
const runningPage = await requestJson("/v2/library-domains/running/activities?locale=en&limit=8");
assert(runningPage.response.ok && runningPage.payload?.data?.length === 8, "Running Library retrieval failed.");
assertLibraryAuthority(runningPage.payload);
const emptyAlternative = await findAlternativeCase("running", runningPage.payload.data, (data) => data.length === 0);
assert(emptyAlternative, "No legitimate empty alternatives result was proven.");
const runningDetail = await requestJson(`/v2/library-domains/running/activities/${runningPage.payload.data[0].revisionId}?locale=en`);
...
const strengthOnlyCandidates = [...page1.payload.data, ...page2.payload.data];
const homeMembershipChecks = await Promise.all(strengthOnlyCandidates.map(async (item) => {
...
const strengthOnly = homeMembershipChecks.find(({ status }) => status === 404);
...
console.log(JSON.stringify({
  ...
  catalogDeploymentId: EXPECTED_CATALOG_DEPLOYMENT_ID,
  ...
  genericDomain: runningPage.payload.data[0].slug,
  ...
  invalidBearerStatus: invalidAuth.response.status,
