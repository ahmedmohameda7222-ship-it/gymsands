...
const outputDir = path.resolve(process.env.QA_TRAIN_EVIDENCE_DIR
...
const directExerciseName = "Barbell squat with a deliberately long activity name for responsive verification";
const snapshotId = "21000000-0000-4000-8000-000000000001";
const itemId = "22000000-0000-4000-8000-000000000001";
const observations = [];
...
function catalogPayload(url) {
  ...
  if (url.pathname.endsWith("/filters")) {
    return { data: { sports: [], activityTypes: [activity.activityType], sessionTypes: [], sessionPhases: [], equipment: activity.equipment, trainingGoals: [], difficulties: ["intermediate"] }, meta };
  }
...
async function installFixture(context, { direct, language, theme, delayCanonical }) {
  ...
  const sets = [1, 2].map((setOrder) => ({
    id: `23000000-0000-4000-8000-00000000000${setOrder}`,
  ...
  }, { languageValue: language, themeId: theme === "dark" ? "elite-noir" : "olive" });

  await context.route("**/api/activity-catalog/**", async (route) => {
    await route.fulfill({ status: 200, contentType: "application/json", headers: { "cache-control": "private, no-store", "x-plaivra-qa-fixture": "aw5-catalog" }, body: JSON.stringify(catalogPayload(new URL(route.request().url()))) });
  });
  await context.route(/\/api\/workouts\/sessions\/[^/]+\/muscle-analysis(?:\?.*)?$/, async (route) => {
    await route.fulfill({ status: 200, contentType: "application/json", headers: { "cache-control": "private, no-store", "x-plaivra-qa-fixture": "aw5-heat-map" }, body: JSON.stringify({
      sessionId, snapshotId, snapshotSchemaVersion: "workout_session_muscle_snapshot_v1",
      frozenAt: "2026-07-27T08:00:00.000Z", source: "session_start",
      snapshotCompleteness: "complete", reasonCodes: [], effectiveCompleteness: "complete", effectiveWarnings: [],
      analysis: { schemaVersion: "muscle_analysis_result_v1", taxonomyVersion: "muscle_taxonomy_v1", engineVersion: "muscle_load_resistance_sets_v1", thresholdVersion: "muscle_load_thresholds_v1", mode: "planned", period: { kind: "session" }, completeness: "complete", muscles: [], contributionBreakdown: [], mappingVersionsUsed: [], coverage: { totalItemCount: 1, includedItemCount: 1, unmappedItemCount: 0, unsupportedItemCount: 0 }, warnings: [] }
    }) });
  });
  ...
  await context.route(/^https:\/\/[^/]+\.supabase\.co\//, async (route) => {
    ...
    if (method === "POST" && pathname.includes("/rest/v1/rpc/start_or_resume_workout_session_atomic")) return respond({ session: root, resumed: true });
    if (method === "POST" && pathname.includes("/rest/v1/rpc/start_or_resume_direct_workout_session_atomic")) return respond({ session: root, resumed: true });
    if (method === "GET" && pathname.includes("/rest/v1/workout_sessions")) return respond(wantsObject ? root : [root], 200, { "content-range": "0-0/1" });
    ...
    if (method === "GET" && pathname.includes("/rest/v1/workout_session_muscle_snapshot_items")) return respond([item], 200, { "content-range": "0-0/1" });
    if (method === "GET" && pathname.includes("/rest/v1/workout_session_prescription_sets")) return respond(sets, 200, { "content-range": "0-1/2" });
    if (method === "GET" && (pathname.includes("/rest/v1/workout_session_prescription_metric_targets") || pathname.includes("/rest/v1/workout_performance_metric_definitions") || pathname.includes("/rest/v1/exercise_logs") || pathname.includes("/rest/v1/user_exercise_alternatives") || pathname.includes("/rest/v1/user_progression_targets"))) return respond([], 200, { "content-range": "*/0" });
    if (method === "POST" && pathname.includes("/rest/v1/rpc/upsert_workout_set_logs_atomic")) {
      ...
      return respond({ saved: payload?.p_logs?.length ?? 1, deleted: 0 });
    }
    if (method === "POST" && pathname.includes("/rest/v1/rpc/complete_workout_session_atomic")) return respond({ ...root, status: "completed", completed_at: "2026-07-27T09:00:00.000Z" });
    if (pathname.includes("/rest/v1/user_app_settings") && (method === "GET" || method === "HEAD")) {
      return route.fulfill({ status: 200, contentType: "application/json", headers: { "content-range": "0-0/1", "x-plaivra-qa-fixture": "localized-settings" }, body: method === "HEAD" ? "" : JSON.stringify(wantsObject ? settings : [settings]) });
    }
    let body = {};
    if (method !== "GET" && method !== "HEAD") {
      try { body = request.postDataJSON(); } catch { body = {}; }
    }
    return route.fulfill({ status: method === "POST" ? 201 : 200, contentType: "application/json", headers: { "content-range": "0-0/0", "x-plaivra-qa-fixture": "aw5-empty" }, body: method === "HEAD" ? "" : JSON.stringify(method === "GET" ? [] : body) });
  });
}
...
async function frameworkChrome(page) {
  return page.evaluate(() => {
    ...
    for (const portal of document.querySelectorAll("nextjs-portal")) {
      ...
      const overlay = root.querySelector("nextjs-errors-dialog, nextjs-error-dialog, [data-nextjs-error-dialog], [data-nextjs-dialog-overlay], [data-nextjs-error-overlay]");
...
async function geometry(page) {
  return page.evaluate(() => {
    ...
    const rects = (selector) => [...document.querySelectorAll(selector)].filter(visible).map((element) => {
      const box = element.getBoundingClientRect();
      return { left: box.left, right: box.right, top: box.top, bottom: box.bottom, width: box.width, height: box.height };
    });
    return {
      viewport: { width: innerWidth, height: innerHeight },
      close: rect("[data-workout-session-close]"), heatMap: rect("[data-aw5-mini-heat-map-slot]"),
      sessionTitle: rect("[data-aw5-session-title]"), metadata: rect("[data-aw5-metadata]"), pause: rect("[data-aw5-pause-resume]"),
      sticky: rect("[data-aw5-sticky-actions]"), reps: rect("#active-set-reps"), weight: rect("#active-set-weight"),
      details: rect("[data-active-set-details-trigger]"), setPath: rect("[data-aw5-set-path]"), feedback: rect("[data-aw5-feedback]"),
      restPresets: rects("[data-aw5-rest-presets] button"),
      horizontalOverflowPx: Math.max(0, document.documentElement.scrollWidth - document.documentElement.clientWidth),
      headings: [...document.querySelectorAll("h1,h2")].filter(visible).map((element) => ({ text: element.textContent?.replace(/\s+/g, " ").trim() ?? "", height: element.getBoundingClientRect().height }))
    };
  });
}
...
function geometryFailures(metrics, options = {}) {
  const failures = [];
  for (const [name, target] of [["Mini Heat Map", metrics.heatMap], ["session title", metrics.sessionTitle], ["metadata", metrics.metadata], ["Pause/Resume", metrics.pause]]) {
    if (overlaps(metrics.close, target)) failures.push(`close intersects ${name}`);
  }
  for (const [name, target] of [["reps input", metrics.reps], ["weight input", metrics.weight], ["details trigger", metrics.details], ["set path", metrics.setPath], ["validation feedback", metrics.feedback]]) {
    if (overlaps(metrics.sticky, target)) failures.push(`sticky intersects ${name}`);
  }
  if (options.restPresets) metrics.restPresets.forEach((target, index) => { if (overlaps(metrics.sticky, target)) failures.push(`sticky intersects rest preset ${index + 1}`); });
  ...
  if (options.direct) {
    if (metrics.headings.some((heading) => /^Start\b/i.test(heading.text) && heading.height > 32)) failures.push("loaded direct route contains a route-level Start heading");
    const count = metrics.headings.filter((heading) => heading.text === directExerciseName).length;
    if (count !== 1) failures.push(`direct exercise heading count is ${count}, expected 1`);
  }
...
async function openSession({ name, route = dayRoute, viewport, language = "en", theme = "light", delayCanonical = false }) {
  ...
  return { name, direct, context, page, response, pageErrors, consoleErrors, consoleWarnings };
}
...
async function record(session, options = {}) {
  ...
  const warnings = session.consoleWarnings.filter((message) => !/Reduced Motion enabled on your device/i.test(message));
  const failures = [
  ...
  observations.push({ name: session.name, route: session.page.url(), status: session.response?.status() ?? null, artifact, metrics, chrome, failures });
  await session.context.close();
}

const visiblePrimary = (page) => page.locator("[data-aw5-primary-action]:visible").first();
...
for (const scenario of [
  ...
  if (session.direct) {
    const label = (await session.page.locator("[data-aw5-session-title]").innerText()).trim();
    if (label !== "Workout session") session.consoleErrors.push(`direct session label is ${JSON.stringify(label)}`);
  }
  await record(session, scenario.options);
}

{
  const session = await openSession({ name: "plan-day-paused-en-390x844", viewport: { width: 390, height: 844 } });
...
{
  const session = await openSession({ name: "plan-day-validation-error-en-390x844", viewport: { width: 390, height: 844 } });
...
{
  const session = await openSession({ name: "plan-day-busy-en-390x844", viewport: { width: 390, height: 844 }, delayCanonical: true });
  ...
  const busy = await session.page.evaluate(() => ({ reps: document.querySelector("#active-set-reps")?.disabled, weight: document.querySelector("#active-set-weight")?.disabled, text: document.querySelector("[data-aw5-execution-shell]")?.textContent ?? "" }));
  if (!busy.reps || !busy.weight) session.consoleErrors.push("busy completion did not disable the editor");
  if (/Saving\.\.\.|Saved/i.test(busy.text)) session.consoleErrors.push("busy completion exposed save-state chrome");
  await record(session);
}
{
  const session = await openSession({ name: "plan-day-rest-en-390x844", viewport: { width: 390, height: 844 } });
  await enterSet(session.page);
  await visiblePrimary(session.page).click();
...
{
  const session = await openSession({ name: "plan-day-session-review-en-1440x900", viewport: { width: 1440, height: 900 } });
  await session.page.getByRole("button", { name: /^Finish$/i }).click();
  await session.page.waitForSelector("[data-aw5-session-review]");
  await record(session);
}
{
  const session = await openSession({ name: "plan-day-completed-summary-en-1440x900", viewport: { width: 1440, height: 900 } });
  await enterSet(session.page);
  await visiblePrimary(session.page).click();
  await session.page.waitForSelector('[data-aw5-session-state="rest"]', { timeout: 20_000 });
  await visiblePrimary(session.page).click();
...
for (const keyboard of ["reps", "weight"]) {
  const session = await openSession({ name: `plan-day-keyboard-${keyboard}-en-390x844`, viewport: { width: 390, height: 844 } });
...
await writeFile(path.join(outputDir, "aw5-correction-layout-qa-results.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
if (failures.length) {
