...
test("Train mock and rendered QA share one stable persisted-set identity contract", () => {
  ...
  assert.match(baseSource, /train-mock-contract\.json/);
  assert.doesNotMatch(baseSource, /function nextId/);
});
...
test("current Train base proves hydration and canonical session-store projection", () => {
  ...
  assert.match(baseSource, /\/rest\/v1\/workout_session_muscle_snapshots/);
  assert.match(baseSource, /\/rest\/v1\/workout_session_muscle_snapshot_items/);
  assert.match(baseSource, /\/rest\/v1\/workout_session_prescription_sets/);
  assert.match(baseSource, /\/rest\/v1\/workout_session_prescription_metric_targets/);
  assert.match(baseSource, /\/rest\/v1\/workout_performance_metric_definitions/);
});
