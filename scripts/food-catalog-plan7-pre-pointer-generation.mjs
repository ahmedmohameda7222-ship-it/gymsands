...
import { validateGenerationSemanticSnapshot } from "../lib/food-catalog/generation-validation-core.ts";
import { computeGenerationCompositionChecksum } from "../lib/food-catalog/generation-composition.ts";
...
BEGIN
  ...
  IF v_generation_checksum <> '${expected}' THEN
    RAISE EXCEPTION 'Plan7 stored generation checksum differs from portable canonical authority';
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM public.food_catalog_generation_validation_reports r
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM public.food_catalog_generation_events e
...
export function loadCanonicalGenerationValidationSnapshot(databaseUrl, generationId) {
  const generation = uuid(generationId, "current generation");
  const generationRow = queryJson(databaseUrl, `
SELECT jsonb_build_object(
  ...
  'compositionChecksumSha256', composition_checksum_sha256,
  'sealedAt', sealed_at::text
)::text
FROM public.food_catalog_generations
WHERE id='${generation}'::uuid;`);

  const foods = jsonArray(databaseUrl, `
SELECT coalesce(jsonb_agg(jsonb_build_object(
  'generationId', generation_id::text,
  'foodId', food_id::text,
  'lifecycle', lifecycle,
  'nutritionRevisionId', nutrition_revision_id::text,
  'activationSetId', activation_set_id::text,
  'activationSetMemberId', activation_set_member_id::text,
  'activationGrantEventId', activation_grant_event_id::text
) ORDER BY food_id), '[]'::jsonb)::text
...
WHERE generation_id='${generation}'::uuid;`);
  ...
  const selectionsByFoodId = Object.fromEntries(foods.map((food) => [food.foodId, {
    servingOptionIds: [],
    nameFactIds: [],
  ...
  const nutritionRevisions = jsonArray(databaseUrl, `
SELECT coalesce(jsonb_agg(jsonb_build_object('id',nr.id::text,'foodId',nr.food_id::text) ORDER BY nr.id),'[]'::jsonb)::text
FROM public.food_catalog_generation_foods gf
JOIN public.food_nutrition_revisions nr ON nr.id=gf.nutrition_revision_id
WHERE gf.generation_id='${generation}'::uuid AND gf.nutrition_revision_id IS NOT NULL;`);
  const servingOptions = jsonArray(databaseUrl, `
SELECT coalesce(jsonb_agg(jsonb_build_object('id',s.id::text,'foodId',s.food_id::text) ORDER BY s.id),'[]'::jsonb)::text
FROM public.food_catalog_generation_servings gs
JOIN public.food_serving_options s ON s.id=gs.serving_option_id
WHERE gs.generation_id='${generation}'::uuid;`);
  const names = jsonArray(databaseUrl, `
SELECT coalesce(jsonb_agg(jsonb_build_object('id',n.id::text,'foodId',n.food_id::text,'role',n.name_role) ORDER BY n.id),'[]'::jsonb)::text
FROM public.food_catalog_generation_names gn
JOIN public.food_names n ON n.id=gn.name_fact_id
WHERE gn.generation_id='${generation}'::uuid;`);
  const taxonomyAssignments = jsonArray(databaseUrl, `
SELECT coalesce(jsonb_agg(jsonb_build_object('id',a.id::text,'foodId',a.food_id::text,'action',a.assignment_action) ORDER BY a.id),'[]'::jsonb)::text
FROM public.food_catalog_generation_taxonomy gt
JOIN public.food_taxonomy_assignments a ON a.id=gt.taxonomy_assignment_id
WHERE gt.generation_id='${generation}'::uuid;`);
  const marketAssignments = jsonArray(databaseUrl, `
SELECT coalesce(jsonb_agg(jsonb_build_object('id',a.id::text,'foodId',a.food_id::text,'action',a.assignment_action) ORDER BY a.id),'[]'::jsonb)::text
FROM public.food_catalog_generation_markets gm
JOIN public.food_market_assignments a ON a.id=gm.market_assignment_id
WHERE gm.generation_id='${generation}'::uuid;`);
  const verificationAssertions = jsonArray(databaseUrl, `
SELECT coalesce(jsonb_agg(jsonb_build_object('id',a.id::text,'foodId',a.food_id::text,'scope',a.assertion_scope,'state',a.assertion_state) ORDER BY a.id),'[]'::jsonb)::text
FROM public.food_catalog_generation_verification gv
JOIN public.food_verification_assertions a ON a.id=gv.assertion_id
WHERE gv.generation_id='${generation}'::uuid;`);
  const activationAuthorities = jsonArray(databaseUrl, `
SELECT coalesce(jsonb_agg(jsonb_build_object(
  'activationSetId',s.id::text,
  'activationSetMemberId',m.id::text,
  'foodId',m.food_id::text,
  'activationPolicyVersion',s.activation_policy_version,
  'eligibility',m.eligibility,
  'sourceLegalAccepted',m.source_legal_accepted,
  'grantEventId',g.id::text,
  'grantCreatedAt',g.created_at::text,
  'invalidatedAt',i.created_at::text
) ORDER BY m.id), '[]'::jsonb)::text
FROM public.food_catalog_generation_foods gf
JOIN public.food_catalog_activation_set_members m
  ON m.id=gf.activation_set_member_id AND m.activation_set_id=gf.activation_set_id
JOIN public.food_catalog_activation_sets s ON s.id=gf.activation_set_id
JOIN public.food_catalog_activation_events g
  ON g.id=gf.activation_grant_event_id AND g.activation_set_id=gf.activation_set_id AND g.event_type='grant'
LEFT JOIN public.food_catalog_activation_events i
  ON i.target_grant_event_id=g.id AND i.activation_set_id=g.activation_set_id AND i.event_type='invalidate'
WHERE gf.generation_id='${generation}'::uuid AND gf.lifecycle='active';`);
  const redirects = jsonArray(databaseUrl, `
SELECT coalesce(jsonb_agg(jsonb_build_object('generationId',generation_id::text,'sourceFoodId',source_food_id::text,'targetFoodId',target_food_id::text) ORDER BY source_food_id),'[]'::jsonb)::text
FROM public.food_catalog_generation_redirects
WHERE generation_id='${generation}'::uuid;`);
...
export function verifyCanonicalPrePointerGeneration({ databaseUrl, generationId, eventId, reportId, expectedChecksum }) {
  ...
  if (semantic.recomputedChecksum !== expected) {
    throw new Error("Plan7 canonical Plan3 recomputed checksum differs from portable canonical authority.");
  }
  runPsql(databaseUrl, buildStoredValidationAuthoritySql({ generationId, eventId, reportId, expectedChecksum: expected }));
  return Object.freeze({
    verified: true,
    recomputedChecksum: semantic.recomputedChecksum,
    blockerCount: semantic.blockerCount,
    verificationStateCount: semantic.verificationStates.length,
  });
}

// Keep the canonical Plan 3 checksum authority reachable from this executable module so
// focused contracts can prove that pre-pointer verification is not a stored-checksum-only gate.
