...
import { publishRecipeDraft } from "@/services/nutrition-v1/server/recipes";
...
export async function POST(request: Request, { params }: { params: Promise<{ recipeId: string }> }) {
  ...
  try {
    ...
    return nutritionJson(await publishRecipeDraft(context.supabase, context.user.id, recipeId));
  } catch (error) {
