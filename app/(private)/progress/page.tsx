...
import type { BodyMeasurement, CanonicalWorkoutActivity, DailyNutritionSummary, ProgressEntry } from "@/types";
...
export default function ProgressPage() {
  ...
  const [workoutActivity, setWorkoutActivity] = useState<CanonicalWorkoutActivity[]>([]);
  ...
  const loadProgress = useCallback(async () => {
    ...
    if (activityResult.status === "fulfilled") {
      setWorkoutActivity(activityResult.value.activities);
      if (Object.values(activityResult.value.sources).some((source) => source.state !== "loaded")) {
        issues.push("Workout consistency is using partial history.");
      }
    } else {
      setWorkoutActivity([]);
  ...
  const completedCount = workoutActivity.filter((session) => session.lifecycle === "completed" || session.lifecycle === "partial").length;
  const skippedCount = workoutActivity.filter((session) => session.lifecycle === "skipped").length;
...
function buildWeeklyInsights({ nutritionWeek, workoutActivity, entries, today }: { nutritionWeek: DailyNutritionSummary[]; workoutActivity: CanonicalWorkoutActivity[]; entries: ProgressEntry[]; today: string }) { const start = startOfWeek(today); const end = addDays(start, 6); const weekSessions = workoutActivity.filter((session) => { const date = session.effectiveAt.slice(0, 10); return date >= start && date <= end; }); const completedWorkouts = weekSessions.filter((session) => session.lifecycle === "completed" || session.lifecycle === "partial").length; const skippedWorkouts = weekSessions.filter((session) => session.lifecycle === "skipped").length; const daysWithLogs = nutritionWeek.filter((day) => day.logs?.length); const waterDays = nutritionWeek.filter((day) => Number(day.water_ml) > 0); const currentWeekProgressEntries = entries.filter((entry) => entry.entry_date >= start && entry.entry_date <= end).length; const totalCalories = daysWithLogs.reduce<number | null>((sum, day) => sum === null || typeof day.calories !== "number" || !Number.isFinite(day.calories) ? null : sum + day.calories, 0); const totalProtein = daysWithLogs.reduce<number | null>((sum, day) => sum === null || typeof day.protein_g !== "number" || !Number.isFinite(day.protein_g) ? null : sum + day.protein_g, 0); const calories = daysWithLogs.length && totalCalories !== null ? Math.round(totalCalories / daysWithLogs.length) : null; const calendarAverageCalories = totalCalories === null ? null : Math.round(totalCalories / 7); const protein = daysWithLogs.length && totalProtein !== null ? Math.round(totalProtein / daysWithLogs.length) : null; const calendarAverageProtein = totalProtein === null ? null : Math.round(totalProtein / 7); const water = waterDays.length ? Math.round(waterDays.reduce((sum, day) => sum + Number(day.water_ml), 0) / waterDays.length) : null; const workoutTotal = completedWorkouts + skippedWorkouts; const workoutCompletionRate = workoutTotal ? Math.round((completedWorkouts / workoutTotal) * 100) : null; const consistencyScore = Math.min(100, (workoutCompletionRate ?? 0) * 0.5 + Math.min(25, daysWithLogs.length * 4) + Math.min(15, waterDays.length * 3) + Math.min(10, currentWeekProgressEntries * 5)); return { completedWorkouts: workoutTotal ? completedWorkouts : null, skippedWorkouts: workoutTotal ? skippedWorkouts : null, weekWorkoutTotal: workoutTotal, weekWorkoutCompletionRate: workoutCompletionRate, averageCalories: calories, calendarAverageCalories, averageProtein: protein, calendarAverageProtein, waterAverage: water, consistencyScore: Math.round(consistencyScore) }; }
