...
export default function GlobalError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    reportClientError({ error, digest: error.digest, boundarySource: "global" });
  }, [error]);
      ...
      <body className="font-sans">
        <main className="premium-page-bg flex min-h-screen items-center justify-center p-4">
          <div className="w-full max-w-md rounded-[22px] bg-card p-6 text-center shadow-soft">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">A privacy-safe error signal was recorded. No workout, nutrition, profile, credential, or prompt content was included.</p>
