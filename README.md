...
- AW-4 provides the shared deterministic session engine, serialized command dispatcher, official Active Workout store, and timestamp-projected timer engine.
- Plaivra Production contains 74 physical migration records. The ledger classifies 63 exact applications and 11 generated-version aliases; `20260726075737_active_workout_aw4_session_engine.sql` is the one committed pending migration.
...
- AW-4 remains an unmerged release candidate. No manual deployment or compatibility-marker promotion is authorized by repository state alone.
