...
describe("Nutrition V1 Add To handoff parsing", () => {
  ...
  it("preserves exact Recipe version identity for Diary, Meal Plan and Saved Meal", () => {
    const params = "source=recipe&recipeId=r1&recipeVersionId=v2";
    expect(parseAddToHandoff(new URLSearchParams(params), "diary")).toEqual({ type: "recipe", id: "r1", versionId: "v2" });
    expect(parseAddToHandoff(new URLSearchParams(params), "meal_plan")).toEqual({ type: "recipe", id: "r1", versionId: "v2" });
    expect(parseAddToHandoff(new URLSearchParams(`${params}&destination=saved_meal`), "saved_meal")).toEqual({ type: "recipe", id: "r1", versionId: "v2" });
