...
const copy = {
  ...
  ar: {
    ...
    sourceDate: "تاريخ المصدر", loadSource: "تحميل اليوم", selectItems: "اختر العناصر", sourceEmpty: "لا يوجد طعام مسجل في هذا اليوم", sourceFailed: "تعذر تحميل هذا اليوم", duplicatesFound: "بعض العناصر المحددة موجودة بالفعل في اليوم المستهدف", copySelected: "نسخ العناصر المحددة", copied: "تم نسخ العناصر",
...
export type EatKey = keyof typeof copy.en;

export function useEatTranslation() {
  const { language, dir } = useTranslation();
  const locale = language === "de" ? "de-DE" : language === "ar" ? "ar-EG" : "en-GB";
  function et(key: EatKey, values?: Record<string, string | number>) {
