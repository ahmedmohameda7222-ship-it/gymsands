...
describe("temporary ChatGPT Developer Mode setup", () => {
  ...
  const setupPage = source(
    "app/(private)/settings/connections/chatgpt/page.tsx",
  );
  ...
  it("is controlled by one public feature flag and is hidden by default", () => {
    expect(environment).toMatch(
      /manualChatGptSetupEnabled:\s*process\.env\.NEXT_PUBLIC_ENABLE_MANUAL_CHATGPT_SETUP === "true"/,
    );
    expect(environmentExample).toContain(
      "NEXT_PUBLIC_ENABLE_MANUAL_CHATGPT_SETUP=false",
    );
  ...
  it("handles a missing public MCP URL without exposing a fallback", () => {
    ...
    expect(component).toMatch(
      /aria-label="Copy MCP server URL"[\s\S]*?disabled/,
    );
