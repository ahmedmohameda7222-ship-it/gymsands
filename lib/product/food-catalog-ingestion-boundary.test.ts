...
const PLAN1_CORRECTION_MIGRATION = "20260901174500_food_catalog_plan1_semantic_corrections.sql";
...
describe("Food Catalog Batch 0 ingestion boundary", () => {
  it("preserves finalized Batch 0 authority while recording its Production alias and the current pending correction", () => {
    ...
    expect(currentPendingEntries).toEqual([
      expect.objectContaining({
        localFile: PLAN1_CORRECTION_MIGRATION,
        state: "pending",
      }),
    ]);
