...
const toolDefinitions: McpToolSource[] = [
  ...
  { name: "get_ai_action_requests", title: "Get AI action requests", description: "Return user-created, reviewable Plaivra requests prepared for ChatGPT. Requires broad user-approved access because request context can span multiple areas.", inputSchema: objectSchema({ status: { type: "string", enum: ["draft", "ready_for_chatgpt", "sent_to_chatgpt", "resolved", "cancelled"] } }), risk: "read" },
  { name: "create_ai_action_request", title: "Create AI action request", description: "Create a reviewable request record only. This does not generate or apply a workout or meal plan.", inputSchema: objectSchema({ action_type: { type: "string" }, source_type: { type: "string" }, source_id: { type: "string" }, context_json: { type: "object", additionalProperties: true }, user_note: { type: "string" } }, ["action_type", "source_type", "context_json"]), risk: "low" },
  { name: "update_ai_action_request_status", title: "Update AI action request status", description: "Update the lifecycle status of a user-owned AI action request without changing plan data.", inputSchema: objectSchema({ request_id: { type: "string" }, status: { type: "string", enum: ["draft", "ready_for_chatgpt", "sent_to_chatgpt", "resolved", "cancelled"] } }, ["request_id", "status"]), risk: "low" },

  { name: "get_safety_profile", title: "Get safety profile", description: "Return user-provided safety context. This is not a diagnosis or medical recommendation.", inputSchema: emptyInputSchema, risk: "read" },
  { name: "update_safety_profile", title: "Update safety profile", description: "Save user-provided safety context and caution level. Do not infer diagnoses.", inputSchema: objectSchema({ injuries: { type: "array", items: { type: "string" } }, pain_areas: { type: "array", items: { type: "string" } }, medical_conditions: { type: "string" }, doctor_restrictions: { type: "string" }, medications_or_supplement_notes: { type: "string" }, pregnancy_or_postpartum: { type: "string" }, eating_disorder_risk_acknowledged: { type: "boolean" }, under_18_flag: { type: "boolean" }, movement_restrictions: { type: "string" }, nutrition_restrictions: { type: "string" }, risk_level: { type: "string", enum: ["green", "yellow", "red"] }, emergency_warning_acknowledged: { type: "boolean" } }), risk: "medium" },
  { name: "get_nutrition_preference_profile", title: "Get nutrition preference profile", description: "Return optional budget, cooking, cuisine, allergy, and grocery preferences.", inputSchema: emptyInputSchema, risk: "read" },
  { name: "update_nutrition_preference_profile", title: "Update nutrition preference profile", description: "Save user-provided meal-planning constraints without generating a meal plan.", inputSchema: objectSchema({ weekly_food_budget: { type: "number" }, budget_currency: { type: "string" }, max_cooking_time_minutes: { type: "number" }, meal_prep_days: { type: "array", items: { type: "string" } }, cooking_skill: { type: "string" }, kitchen_equipment: { type: "array", items: { type: "string" } }, preferred_cuisines: { type: "array", items: { type: "string" } }, disliked_foods: { type: "array", items: { type: "string" } }, allergies: { type: "string" }, repeat_tolerance: { type: "string" }, meals_per_day: { type: "number" }, ingredient_reuse_preference: { type: "string" }, grocery_style_preference: { type: "string" } }), risk: "medium" },

  { name: "get_progression_targets", title: "Get progression targets", description: "Return saved next-session targets provided by a user or ChatGPT.", inputSchema: objectSchema({ plan_exercise_id: { type: "string" } }), risk: "read" },
  ...
  { name: "create_exercise_alternative", title: "Create exercise alternative", description: "Save a specific user-approved or ChatGPT-provided exercise alternative.", inputSchema: objectSchema({ plan_exercise_id: { type: "string" }, original_exercise_name: { type: "string" }, alternative_exercise_name: { type: "string" }, reason: { type: "string" }, target_muscle: { type: "string" }, equipment: { type: "string" }, pain_friendly_note: { type: "string" }, created_by: { type: "string", enum: ["user", "chatgpt"] } }, ["plan_exercise_id", "original_exercise_name", "alternative_exercise_name", "reason"]), risk: "medium" },

  { name: "get_grocery_items", title: "Get grocery items", description: "Return persisted ingredient-level grocery items for a week.", inputSchema: objectSchema({ week_start: isoDate }, ["week_start"]), risk: "read" },
  { name: "upsert_grocery_item", title: "Upsert grocery item", description: "Create or update one user-owned grocery item supplied explicitly by the user or ChatGPT.", inputSchema: objectSchema({ grocery_item_id: { type: "string" }, week_start: isoDate, source_meal_plan_item_id: { type: "string" }, item_name: { type: "string" }, quantity: { type: "number" }, unit: { type: "string" }, store_section: { type: "string", enum: ["Protein", "Carbs", "Vegetables", "Fruits", "Dairy", "Pantry", "Frozen", "Drinks", "Other"] }, checked: { type: "boolean" }, already_have: { type: "boolean" }, notes: { type: "string" }, created_by: { type: "string", enum: ["manual", "meal_plan", "chatgpt"] } }, ["week_start", "item_name"]), risk: "low" },
  { name: "get_daily_checkins", title: "Get daily check-ins", description: "Return morning and evening check-ins for a date range.", inputSchema: objectSchema({ start_date: isoDate, end_date: isoDate }, ["start_date"]), risk: "read" },
  { name: "upsert_daily_checkin", title: "Upsert daily check-in", description: "Save one explicit morning or evening check-in.", inputSchema: objectSchema({ checkin_date: isoDate, checkin_type: { type: "string", enum: ["morning", "evening"] }, sleep_hours: { type: "number" }, energy_level: { type: "string" }, soreness_level: { type: "string" }, stress_level: { type: "string" }, motivation_level: { type: "string" }, workout_readiness: { type: "string" }, today_main_goal: { type: "string" }, today_blocker: { type: "string" }, workout_done: { type: "boolean" }, protein_hit: { type: "boolean" }, calories_hit: { type: "boolean" }, water_hit: { type: "boolean" }, steps_or_movement_done: { type: "boolean" }, meal_plan_followed: { type: "boolean" }, main_blocker: { type: "string" }, tomorrow_note: { type: "string" } }, ["checkin_date", "checkin_type"]), risk: "low" },
  { name: "get_nutrition_target_profiles", title: "Get nutrition target profiles", description: "Return user- or ChatGPT-supplied default, training, rest, and high-activity targets.", inputSchema: emptyInputSchema, risk: "read" },
  { name: "upsert_nutrition_target_profile", title: "Upsert nutrition target profile", description: "Store an explicit nutrition target profile. Plaivra does not generate target values.", inputSchema: objectSchema({ target_type: { type: "string", enum: ["default_day", "training_day", "rest_day", "high_activity_day"] }, calories: { type: "number" }, protein_g: { type: "number" }, carbs_g: { type: "number" }, fat_g: { type: "number" }, water_ml: { type: "number" }, notes: { type: "string" } }, ["target_type"]), risk: "medium" },
