...
const toolDefinitions: McpToolSource[] = [
  ...
  { name: "add_food_log", title: "Log eaten food", description: "Add food the user says they ate directly to Food Log as completed.", inputSchema: objectSchema({ meal_type: { type: "string", enum: ["Breakfast", "Lunch", "Dinner", "Snack"] }, date: isoDate, items: arrayOf({ ...foodInput, serving_option_id: uuid }, ["food_name", "quantity"]), notes: { type: "string" } }, ["meal_type", "items"]), risk: "low" },
  ...
  { name: "create_custom_meal", title: "Create saved meal", description: "Create a user-owned saved meal from explicit food items.", inputSchema: objectSchema({ meal_name: { type: "string" }, meal_category: { type: "string" }, items: arrayOf({ ...foodInput, serving_option_id: uuid }, ["food_name", "quantity"]), notes: { type: "string" } }, ["meal_name", "items"]), risk: "medium" },
