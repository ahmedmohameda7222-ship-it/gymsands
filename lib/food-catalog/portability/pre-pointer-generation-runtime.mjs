...
import { computeGenerationValidationReportChecksum } from "../generation-validation-report.ts";
...
export function loadStoredGenerationValidationReport(databaseUrl, reportId) {
  const report = uuid(reportId, "current validation report");
  return queryJson(databaseUrl, `SELECT jsonb_build_object(
    'generationId',r.generation_id::text,
    'generationChecksumSha256',r.generation_checksum_sha256,
    'validatorSetVersion',r.validator_set_version,
    'policyVersion',r.policy_version,
    'reportChecksumSha256',r.report_checksum_sha256,
    ...
    'findings',coalesce((
      SELECT jsonb_agg(jsonb_build_object(
        'findingOrdinal',f.finding_ordinal,
        'reasonCode',f.reason_code,
        'foodId',f.food_id::text,
        'severity',f.severity,
        'blocking',f.blocking,
        'evidenceReference',f.evidence_reference,
        'validatorPolicyVersion',f.validator_policy_version,
        'details',f.details
      ) ORDER BY f.finding_ordinal)
      FROM public.food_catalog_generation_validation_findings f
      WHERE f.report_id=r.id
    ),'[]'::jsonb)
  )::text FROM public.food_catalog_generation_validation_reports r WHERE r.id='${report}'::uuid;`);
}
...
export function verifyCanonicalPrePointerGeneration({ databaseUrl, generationId, eventId, reportId, expectedChecksum }) {
  ...
  const storedReport = loadStoredGenerationValidationReport(databaseUrl, reportId);
  const storedReportChecksum = checksum(storedReport.reportChecksumSha256, "stored validation report semantic checksum");
  const recomputedReportChecksum = computeGenerationValidationReportChecksum({
    generationId: storedReport.generationId,
    generationChecksumSha256: storedReport.generationChecksumSha256,
    validatorSetVersion: storedReport.validatorSetVersion,
    policyVersion: storedReport.policyVersion,
    blockerCount: storedReport.blockerCount,
    errorCount: storedReport.errorCount,
    warningCount: storedReport.warningCount,
    infoCount: storedReport.infoCount,
    findings: storedReport.findings,
    verificationStates: semantic.verificationStates,
  });
  if (recomputedReportChecksum !== storedReportChecksum) {
    throw new Error(`Plan7 validation report semantic checksum mismatch before pointer restore: stored=${storedReportChecksum}, recomputed=${recomputedReportChecksum}.`);
  }
  ...
  return Object.freeze({
    ...
    reportChecksumSha256: storedReportChecksum,
    recomputedReportChecksumSha256: recomputedReportChecksum,
